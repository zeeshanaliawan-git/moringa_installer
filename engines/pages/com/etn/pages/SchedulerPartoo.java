...
public class SchedulerPartoo {
    ...
    public SchedulerPartoo(String parm[]) throws Exception {
        env = new Properties();
        ...
        semaphore = env.getProperty("PARTOO_SEMAPHORE");
    ...
    public void run() {
        ...
        File toStop = new File("_stopPartoo");
        toStop.delete();

        System.out.println("*****************\nStart SchedulerPartoo (" + getTime() + ")");
        System.out.println("Timeout:" + wait_timeout);

        try {

            PartooServices ps = new PartooServices(Etn, env);
            PartooInterface partoo = null;
			if("v2".equalsIgnoreCase(env.getProperty("partoo_api_version"))) 
			{
				System.out.println("We are using Partoo v2 apis");
				partoo = new PartooV2(Etn, env, debug);
			}
			else
			{
				System.out.println("We are using Partoo v1 apis");
				partoo = new Partoo(Etn, env, debug);
			}
            ...
            while (!stop) {
                ...
                while(rsSites.next()){
                    try{
                        ps.processPartoo(rsSites.value("id"));
                    }catch(Exception e){
                        System.out.println("==================Error in processing partoo==================");
                        e.printStackTrace();
                    }
                }
                ...
                try{
                    ...
                    System.out.println("==================Error in process partoo==================");
                    e.printStackTrace();
                }

				Etn.executeCmd("update "+env.getProperty("COMMONS_DB")+".engines_status set end_date=NOW() where engine_name='Partoo'");
            ...
            System.out.println("Stop SchedulerPartoo (" + getTime() + ")");
        }
