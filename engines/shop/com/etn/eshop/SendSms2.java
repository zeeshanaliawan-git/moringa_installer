...
public int send( int wkid, String smsParam , ClientSql etn  )
{
  ...
  while( true ) 
  {
   rs2 = etn.execute("select * from sms where sms_id="+smsid );
   if( !rs2.next() )
   ...
   // No Suiv -> election
   if( ( clause = rs2.value("where_clause") ).length() == 0  ||
  ...
  System.out.println("----------- SMS lang id : " + langId + " code : " + lang);
	String smstxt = rs2.value("texte");
	if(langId.equals("1") == false) smstxt = parseNull(rs2.value("lang_"+langId+"_texte"));
	if(smstxt.length() == 0)//no sms template defined for the language then fall back to the first lang sms text
	{
		smstxt = rs2.value("texte");
	}
