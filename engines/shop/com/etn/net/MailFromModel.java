...
//    Map<String, String> attributesjsonmap;
    Map<String, Object> ordersnapshotjsonmap;
...
//        }
        ...
        i = j = 0;
        while ((i = src.indexOf("dbos", j)) != -1) {
            for (k = i + 4; k < src.length() && isLetterOrDigit(src.charAt(k)); k++);
            String col = src.substring(i + 4, k);

            //for attributes
            if (ordersnapshotjsonmap != null && ordersnapshotjsonmap.get(col) != null) {
                val = parseNull(ordersnapshotjsonmap.get(col));
                src.replace(i, k, val);
                j = i + val.length();
            }
            else {
     ...
     * @return : 0 OK , ou errcode.
     */
    public MimeMessage build(InputStream fmodele, Set rs, String sujet, String attachs[])
            throws Exception {
...
//        }
        ...
        if (!"".equals(parseNull(rs.value("order_snapshot")))) {
            Gson gson = new Gson();

   Type listType = new TypeToken<Map<String,Object>>(){}.getType();
            ordersnapshotjsonmap = gson.fromJson(rs.value("order_snapshot"), listType);
        }
