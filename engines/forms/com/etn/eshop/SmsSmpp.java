...
public class SmsSmpp extends OtherAction
{
	...
	private int send(int wkid, String clid, String smsid)
	{
		...
		try
		{
			...
			Set rs = etn.execute("select * from post_work where id = "+wkid );
			if(!rs.next()) return(NO_ROW);
			
			String formtablename = rs.value("proces");
			
			Set rsF = etn.execute("select * from "+formtablename+" where "+formtablename+"_id = "+escape.cote(clid));
			if(!rsF.next()) return(NO_ROW);
			
			String contactNumber = Util.parseNull(rsF.value("_etn_telephone"));
			...
			if(Util.parseNull(env.getProperty("sms.smpp.host")).length() == 0 ||
				...
				{
					ret = MISSING_CONFIG;
					((Scheduler)env.get("sched")).retry( wkid , clid, ret, "SMPP configurations are missing. Will retry in 30mins", false, "30" );
					return ret;
				}
			...
			log("Final SMS : " + smstxt);
			return sendToSmsc(wkid , clid, contactNumber, smstxt);
		}
		catch(Exception e)
		{
			e.printStackTrace();
			ret = SOME_EXCEPTION;
			((Scheduler)env.get("sched")).retry( wkid , clid, ret, "Some exception while sending sms. Will retry", false, "5" );
		}
		return ret;
	}
	...
	private int sendToSmsc(int wkid, String clid, String phoneNumber, String smstxt)
	{
		...
		try
		{
			...
			SubmitSmResult result = session.submitShortMessage(null,
                    ...
                    new RegisteredDelivery(SMSCDeliveryReceipt.SUCCESS_FAILURE), (byte)0, new GeneralDataCoding(Alphabet.ALPHA_DEFAULT, MessageClass.CLASS1, false), (byte)0, smstxt.getBytes("utf-8"));
			...
			{
				ret = SUCCESS;				
				etn.executeCmd("update post_work set errCode = 0, errMessage= "+escape.cote("SMS sent as messageId:"+messageId)+" where id="+escape.cote(""+wkid) );
				((Scheduler)env.get("sched")).endJob( wkid , clid );
			}
			else
			{
				ret = NO_MESSAGE_ID;
				((Scheduler)env.get("sched")).retry( wkid , clid, ret, "No message ID returned. We should retry message", false, "5" );			
			}
		}
		catch(Exception e)
		{
	...
	public static void main(String[] a) throws Exception
	{
		...
		s.init(Etn, env);
		System.out.println(s.sendToSmsc(0,"0","+255","Test sms"));
	}
}
