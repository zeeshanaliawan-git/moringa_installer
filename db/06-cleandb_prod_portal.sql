...
--
-- Host: localhost    Database: cleandb_prod_portal
-- ------------------------------------------------------
...
INSERT INTO `config` VALUES ('BASE_DIR','/home/asimina/tomcat/webapps/asimina_prodportal/',NULL),('basic_auth_realm','asimina-prod-site',NULL),('BILL_TRACKING_JSP','/cart/defaultTrackingBill.jsp','Set appropriate path for each country if required'),('CALL_BACK_API_SHELL_SCRIPT','/home/asimina/tomcat/webapps/asimina_prodportal/APICallBack/',NULL),('CART_COOKIE','asimina_prodcatalogCartItems',NULL),('CART_EXTERNAL_LINK','/cart/',NULL),('CART_URL','/asimina_prodportal/',NULL),('CATALOG_DB','cleandb_prod_catalog',NULL),('CATALOG_LINK','/asimina_prodcatalog/',NULL),('CHECKOUT_JSP','/cart/personalinfo.jsp',NULL),('COMMONS_DB','cleandb_commons',NULL),('COMMON_RESOURCES_URL','/asimina_pages/uploads/','Path where all local resources are uploaded. Cache will not cache these'),('DOWNLOAD_PAGES_FOLDER','sites/',NULL),('DYNAMIC_PAGES','',NULL),('ESHOP_FRAUD_JSP','/cart/default_error_fraud.jsp','configure this for your shop'),('ESHOP_SERVER_ERROR_JSP','/cart/default_error_server.jsp','configure this for your shop'),('EXTERNAL_LINK','/asimina_prodportal/',NULL),('funnel_documents_base_dir','/home/asimina/tomcat/webapps/asimina_prodshop/uploads/',NULL),('funnel_documents_base_url','/asimina_prodshop/uploads/',NULL),('GENERIC_FORM_DEFAULT_PHASE','formsubmitted',NULL),('GENERIC_FORM_DEFAULT_PROCESS','genericforms',NULL),('IDCHECK_BROKER','',NULL),('IDCHECK_CLIENT_ID','',NULL),('IDCHECK_END_POINT','',NULL),('IDCHECK_GET_TOKEN_END_POINT','',NULL),('IDCHECK_GRANT_TYPE','',NULL),('IDCHECK_PASSWORD','',NULL),('IDCHECK_USERNAME','',NULL),('IDNOW_CIS_BASE_URL','https://api-test.ariadnext.com/gw/cis/',NULL),('IGNORE_RELATIVE_URLS','/asimina_ckeditor/sites/;/asimina_pages/pages/','This is used by process.jsp to know which relative url must go through the process.jsp'),('ignore_urls_for_encode','',NULL),('INTERNAL_API_URLS','/api/;/asimina_portal/api/','Semi colon separated list of internal api urls. Internal APIs is always in portal and not in prodportal'),('IS_PRODUCTION_ENV','1',NULL),('LOCAL_LINK','http://127.0.0.1/asimina_prodportal/',NULL),('LOGGER_LEVEL','info',NULL),('MENU_IMAGES_FOLDER','/home/asimina/tomcat/webapps/asimina_prodportal/menu_resources/uploads/',NULL),('MENU_IMAGES_PATH','/menu_resources/uploads/',NULL),('NOCACHE_JS','/asimina_prodportal/js/contactus.js',NULL),('NO_PROCESS_JS','/asimina_pages/;/asimina_prodcatalog/js/o_share_bar.js;/asimina_forms/js/html_form_template.js;/asimina_forms/js/triggers.js;/asimina_prodcatalog/js/nocache/;/asimina_prodcatalog/js/fullcalendar.min.js;/asimina_prodcatalog/js/newui/;/asimina_catalog/js/nocache/;/asimina_catalog/js/newui',NULL),('PASS_LANG_URLS','http://127.0.0.1/asimina_prodcatalog/;http://127.0.0.1/asimina_forms/',NULL),('payment_return_jsp','payment.jsp','configure this for your shop'),('PORTAL_CONTEXTPATH','/asimina_prodportal/',NULL),('PORTAL_LINK','/',NULL),('PORTAL_SEARCH_SHELL_SCRIPT','/home/asimina/tomcat/webapps/asimina_prodportal/script/',NULL),('PRODUCTS_IMG_PATH','/asimina_prodcatalog/uploads/products/',NULL),('PRODUCTS_UPLOAD_DIRECTORY','/home/asimina/tomcat/webapps/asimina_prodcatalog/uploads/products/',NULL),('PROXY_PREFIX','/asimina_prodportal/301','When in production we have urls like /particuliars /business which are then routed to /2/sites/particuliar /2/sites/business ... so in that case the redirections added must be checked along with proxy prefix'),('PUBLISHER_BASE_DIR','/home/asimina/tomcat/webapps/asimina_prodportal/',NULL),('PUBLISHER_DOWNLOAD_PAGES_FOLDER','publishsites/',NULL),('PUBLISHER_SEND_REDIRECT_LINK','/asimina_prodportal/publishsites/',NULL),('SEND_REDIRECT_LINK','/',NULL),('SEND_SESSION_PARAMS','http://127.0.0.1/asimina_expert_system/',NULL),('SHOP_DB','cleandb_prod_shop',NULL),('SHOP_SEMAPHORE','D006',NULL),('SITEMAP_SEND_REDIRECT_LINK','/asimina_prodportal/sites/',NULL),('SMART_BANNER_ICON_URL','/asimina_prodportal/img/smartbanner/',NULL),('sso_app_id','',NULL),('SUPER_USER_REG_FORM_URL','',NULL),('UPLOADS_FOLDER','uploads/',NULL),('VALID_404_URLS','',NULL),('VARIANT_IMAGE_UPLOAD_DIRECTORY','/home/asimina/tomcat/webapps/asimina_prodshop/variant_images/','for shop');
...
CREATE TABLE `menus` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_menus_lang_variant` (`site_id`,`langue_id`,`variant`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Table structure for table `publish_content`
--
...
CREATE TABLE `publish_content` (
  ...
  `site_id` int(11) NOT NULL,
  `publication_type` enum('publish','unpublish') DEFAULT NULL,
  `ctype` enum('page','structuredcontent','structuredpage','store','catalog','product','forum','marketing_rules','new_product') DEFAULT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `idx1` (`site_id`,`ctype`,`cid`),
  KEY `idx2` (`status`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `publish_content`
--

LOCK TABLES `publish_content` WRITE;
/*!40000 ALTER TABLE `publish_content` DISABLE KEYS */;
/*!40000 ALTER TABLE `publish_content` ENABLE KEYS */;
UNLOCK TABLES;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `person_sites` AS select `person_sites`.`person_id` AS `person_id`,`person_sites`.`site_id` AS `site_id` from `cleandb_portal`.`person_sites` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-12-05 20:21:59
