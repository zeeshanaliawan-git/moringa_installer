...
<%!
	int GetNumberOfOpenOrders(com.etn.beans.Contexte etn, String process, String phase, String customerIdsList)
	{
		...
		{
			String cids = escape.cote(customerIdsList);
			cids = cids.replaceAll(",","','");			
			qry += " and client_key in ("+cids+") ";
		}
	...
	Date GetStartDateOfProcess(com.etn.beans.Contexte etn, String process, String customerIdsList) throws Exception
	{
		...
		if(!customerIdsList.equals(""))
		{
			String cids = escape.cote(customerIdsList);
			cids = cids.replaceAll(",","','");			
			minStartDateQry += " and client_key in ("+cids+") ";
		}
...
{	
	...
	var data =  "process="+process+"&phase="+phase+"&customerIdsList=<%=customerIdsList%>";
	oldjquery.ajax({
		...
		success: function(resp) {
			...
			oldjquery("#modalWindow").dialog("option","title","Details");
			oldjquery("#modalWindow").dialog("option","width",1050);
			oldjquery("#modalWindow").dialog("option","height",450);
...
function viewOrderFlow(process, phase, idPanier)
{
	oldjquery.ajax({
		...
		success: function(resp) {
			resp = oldjquery.trim(resp);			
			oldjquery("#modalWindow").html(resp);
			oldjquery('#modalWindow').bind("dialogclose", function(event, ui) 
			{  
...
<% if(process != null && !process.equals("#") && !process.equals("")) 
	{ %> 			
		...
		<%
			...
			String Qry = "select distinct process, phase, displayName, rulesVisibleTo, visc, ifnull(topLeftX,"+defaultXCoord+") as topLeftX, ifnull(topLeftY,"+defaultYCoord+") as topLeftY , width, height, actors, priority, execute, color from phases where process = " + escape.cote(process )+ " ";
			Set result = Etn.execute(Qry);
		%>
		workflow  = new draw2d.GizmoWorkflow(process, "divGraph","modalWindow");
		if(oldjquery.browser.msie) workflow.isIE = true;
...
//			String val = (String)phases.get(result.value("phase"));
		...
		// Old Code End
		%>
		var groupObj = new GizmoFigureGroup('<%=result.value("phase")%>',"<%=result.value("actors")%>",'<%=result.value("priority")%>','<%=result.value("execute")%>','<%=result.value("color")%>','<%=result.value("visc")%>','<%=(result.value("displayName")).replaceAll("'","&#39;")%>','<%=result.value("process")%>', document.forms[0].process.value);
...
/*				if(!found)
	...
	oldjquery(document).ready(function() {
		oldjquery("#modalWindow").dialog({
			bgiframe: true, autoOpen: false, height: 'auto', width:'auto', modal: true, resizable : false
		});
		oldjquery("#showLabels").click(function(){
			if(oldjquery("#showLabels").is(':checked') && workflow)
			{
				workflow.showConnectionLabels();
			}
			else
		...
		if(oldjquery("#deleteProcessBtn")) {
			oldjquery("#deleteProcessBtn").click(function(){
		...
		if(oldjquery("#copyProcessBtn")) {
			oldjquery("#copyProcessBtn").click(function(){
				...
				oldjquery("#modalWindow").dialog("option","title","Copy Process");
				oldjquery("#modalWindow").dialog("option","width","auto");
				oldjquery("#modalWindow").dialog("option","height","auto");
				oldjquery('#modalWindow').unbind("dialogclose");	   
				oldjquery("#modalWindow").html(html);
				oldjquery("#modalWindow").dialog('open');
			});
			onOkCopyProcess = function ()
			{
				oldjquery("#copyProcessMsgLbl").html("&nbsp;");
				if(oldjquery("#saveAsProcessName").val() == '')
				{
					oldjquery("#copyProcessMsgLbl").html("Enter phase name");
					return;
				}
				oldjquery.ajax({
					url: 'copyProcess.jsp',
					type: 'POST',
					data: {process: document.forms[0].process.value, copyAs: oldjquery('#saveAsProcessName').val()},
					dataType: 'json',
					success: function(resp) {
						...
						{						
							oldjquery("#copyProcessMsgLbl").html(resp.MESSAGE);
						}
					},
					error : function(resp) {
						alert("Some error while communication with server. Try again or contact administrator");
		...
		onNewProcess = function ()
		{
			...
			oldjquery("#modalWindow").dialog("option","title","New Process");
			oldjquery("#modalWindow").dialog("option","width","auto");
			oldjquery("#modalWindow").dialog("option","height","auto");
			oldjquery('#modalWindow').unbind("dialogclose");	   
			oldjquery("#modalWindow").html(html);
		...
		{
			if(oldjquery('#newProcessName').val() == '')
			{
				oldjquery('#newProcessMsg').html("Enter Process name");
				return;
			}		
			oldjquery.ajax({
				url: 'checkProcessExists.jsp',
				type: 'POST',
				data: {process: oldjquery('#newProcessName').val()},
				success: function(resp) {
					resp = oldjquery.trim(resp);
					if(resp.indexOf('SUCCESS') > -1)
					{	
						window.location.reload();
						// var newOpt = new Option(oldjquery('#newProcessName').val(), oldjquery('#newProcessName').val());
						...
						// if(!workflow.isIE) oldjquery('#opacity').show();
					...
					else oldjquery('#newProcessMsg').html(resp);
				},
				error : function(resp) {
					alert("Some error while communication with server. Try again or contact administrator");
				}
		...
		help = function ()
		{
			...
			oldjquery("#modalWindow").dialog("option","title","Help");
			oldjquery("#modalWindow").dialog("option","width","auto");
			oldjquery("#modalWindow").dialog("option","height","auto");
			oldjquery('#modalWindow').unbind("dialogclose");	   
			oldjquery("#modalWindow").html(html);
		...
		oldjquery(".profChkbox").change(function(){
			var eId = this.id;
		...
		oldjquery(".phaseChkbox").change(function(){
			var eId = this.id;
			eId = eId.substring(eId.indexOf("checkbox")+8);
		...
		oldjquery("#opacity").change(function(){
			if(workflow)
			{	
		...
		oldjquery("#opacity").hide();
		oldjquery("#toolBar2").hide();
		if(workflow && !workflow.isIE) 
		{			
			oldjquery("#opacity").show();
		}
		if(workflow) oldjquery("#toolBar2").show();
