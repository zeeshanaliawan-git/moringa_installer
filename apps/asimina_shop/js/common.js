...
function escapeCoteValue(str)
{
	return str.replace(/'/g, "&#39;").replace(/"/g, "&#34;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

function priceformatter(langFormatter, decimalto, roundto, num ){
    ...
    if(langFormatter == "") langFormatter = "us";
    else if (langFormatter ==  "french") langFormatter = "fr";
    else if (langFormatter ==  "german") langFormatter = "de";
    else langFormatter = "us";
    const formatter = new Intl.NumberFormat(langFormatter, {
    minimumFractionDigits: decimalto,
    maximumFractionDigits: decimalto,
    });
    return formatter.format(num.toFixed(roundto));
}
