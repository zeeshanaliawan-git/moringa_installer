...
<%
	...
	if(action.equals("new"))
	{
		...
		{
			qry= "insert into phases (process, phase, displayName, topLeftX, topLeftY,actors,priority,execute, color, visc, rulesVisibleTo, reverse, oprType) values ("+escape.cote(process)+","+escape.cote(phase)+","+escape.cote(displayName)+","+escape.cote(x)+","+escape.cote(y)+","+escape.cote(actors)+","; 
			if(priority.equals(""))
				qry += "NULL";
			else 
				qry += " "+escape.cote(priority)+" ";
			
			qry += ", "+escape.cote(execute)+", "+escape.cote(color)+","+escape.cote(visible)+","+escape.cote(currentProfile)+","+escape.cote(isReverse)+","+escape.cote(oprType)+")";	
