...
String []getTables( String list )
{
  ...
  //Set rs = qy.set_req_catalog;
  //rs.moveFirst();
  //System.out.println("rs="+new String(rs.rs.data));
  while( rs.next() ){
  ...
  java.util.HashMap h = qy.ctx;
  if(h.get("graphTableDbName")!=null){
	  ar2.add(  h.get("graphTableDbName").toString()+"."+h.get("graphTableName").toString() );
  }
...
boolean set( StringBuffer buf , int id , String list ,int recurs )
{
 ...
 // if(t.length == 0 ) return( false); pour modif : Ajout des jointures pour les tables process 
...
**/
  ...
  */
  ...
  if(t.length==0) buf2.append("'' "); //remplace if(t.length == 0 ) return( false); pour modif : Ajout des jointures pour les tables process 
...
**/
  ...
  //}
	  ...
	  //Ajout des jointures pour les tables process
	  HashMap h = qy.ctx;
	  if(h.get("graphTableJoinClause")!=null){
		  if(!h.get("graphTableJoinClause").toString().equals("")){
			  String extra_join= h.get("graphTableJoinClause").toString();
			  extra_join = extra_join.toLowerCase();
			  extra_join = extra_join.substring(extra_join.indexOf("join")+4);
			  
			  buf.append( (buf.length()==0?"":" join ")+extra_join);
		  }
	  }
	  
	  //Ajout des jointures pour les tables process - fin
...
public String setJoin( int id , String list )
{
  ...
  { qy.setErr( buf.toString() );
    ...
    //System.out.println("setJoin="+buf.toString() );
  return( buf.toString() );
}
...
public Explain explain( int id , String list)
{
  ...
  try
  {
     ...
     CodeXdrSql.encode( xs ,jrs ,  8 /* mysql */ );
     jrs.close();
