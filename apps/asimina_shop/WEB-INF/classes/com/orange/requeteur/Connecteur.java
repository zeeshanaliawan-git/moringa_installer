...
int timeout;
final String opDbg = "connectTimeout=0&useNewIO=true&socketTimeout=0&enableQueryTimeouts=false&useUnbufferedInput=true";
...
throws Exception
{
  return(  DriverManager.getConnection( "jdbc:mysql://"+host, props)); 
}
...
public String[] getDbs()
{
  ...
  ArrayList<String>li = new ArrayList<String>();
  try {
  ResultSet rs = ct[n].getMetaData().getCatalogs();
  while( rs.next() )
    { String s = rs.getString(1);
      if( !s.equalsIgnoreCase("test") &&
          !s.equalsIgnoreCase("mysql") )
...
throws Exception
{
  ...
  StringTokenizer st = new StringTokenizer( cred+"&"+opts+"&"+opDbg, "&" );
...
throws Exception
{
  ...
  Connecteur con = new Connecteur( "127.0.0.1:3406","root",null);
