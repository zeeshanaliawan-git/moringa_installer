...
%>
<%!String AfficheTableau(com.etn.beans.Contexte Etn,javax.servlet.http.HttpServletRequest req,com.etn.lang.ResultSet.Set rs,String aff_valeur,String ligne,String colonne,String valeur,java.util.HashMap options){
...
if( colonne.equals("")){
	if(! aff_valeur.equals("0")){
		buf.append("<br><center><font style='color:red;font-size:11px;'>Vous n'avez pas sélectionné de champ pour l'agrégation par colonnes, l'affichage à été basculé (non enregistré) en mode ligne.<br><br></font></center>");
	}
	aff_valeur = "0";
}
if(aff_valeur.equals("0")){
	if(! colonne.equals("")){
		if( options.get("affiche_total_colonne")!=null){
			if( options.get("affiche_total_colonne").equals("1")){		
				buf.append("<br><center><font style='color:red;font-size:11px;'>Vous avez sélectionné un champ pour l'agrégation par colonnes et un mode ligne pour l'affichage,cela va provoquer une décalage dans les totaux en colonnes.<br><br></font></center>");
			}
		}
	}
}
...
if( aff_valeur.equals("3") || aff_valeur.equals("4") ){
/****************************************************************	TOTAL LIGNE	**********************************************************************/
...
rs = new com.etn.lang.ResultSet.HtmlSet(ex_1.getXdr());
...
if( aff_valeur.equals("0")){
	...
	buf.append("<br><table id='tableau' cellspacing='0' cellpadding='2' border='0'><tr>");
	   ...
	   while( rs.next() )
	   {
	     ...
	     //	out.write("i="+col[i]+"<br>");
		String j= rs.htmlValue(i);
	     	//String j= rs.value(i );
	    if(j==null){
	    	buf.append("<td class='c2'>--</td>" );
		}else{
			buf.append("<td class='c1'>"+("".equals(j)?"--"://j.replace('.',','))+"</td>" );
				 typeValeur2(j,typeValeur,i,rs)
				 )+"</td>" );
		}
	     }
	     buf.append("</tr>");
	   ...
	   /*for( int i = 0 ; i < rs.Cols ; i++ ){
		   buf2.append("'"+rs.ColName[i]+"'<br>");
	   }*/
	   
		if( options.get("affiche_total_colonne")!=null){
				if( options.get("affiche_total_colonne").equals("1")){		

				HashMap total4  = new HashMap();
				
				while(rs.next()){
					for(int j=0;j < valeur2.length;j++){
						//buf2.append("'"+valeur2[j]+"'==>"+rs.value(valeur2[j].trim())+"<br>");
						
						int typV = 2;
						int col = rs.indexOf(valeur2[j].trim());
						//System.out.println("col='"+valeur3+"'====>col="+col + "==>"+rs.types[1] );
						//buf2.append("col='"+valeur3+"'====>col="+col + "==>"+rs.types[col]+"<br>");
						...
						concat_valeur_numeric(total4,valeur2[j],rsHtmlSet(valeur2[j].trim()),typV);				
					}
				}
				buf.append("<tr>");	
				buf.append("<td class='totalbot' colspan='"+vertical2.length+"'>"+libelle_msg(Etn,req,"Total")+"</td>");
				for(int j=0;j < valeur2.length;j++){
					buf.append("<td class='totalbot' id=\"total_"+valeur2[j]+"\">"+typeValeur2(""+total4.get(valeur2[j]),typeValeur,rs.indexOf(valeur2[j]),rs) +"</td>");
					}
				}
			}
		/*********************************************************************	TOTAL COLONNE	***************************************************************/				
		...
		if( rs.rs.Rows == 0){
			buf.append("<tr><td colspan='"+rs.Cols+"'>Pas de résultat</td></tr>");
	   }
		buf.append("</table>");
	   ...
	   com.etn.lang.ResultSet.ArraySet ar = new com.etn.lang.ResultSet.ArraySet(ex1.getXdr());	//vertical
	   com.etn.lang.ResultSet.ArraySet ar2 = new com.etn.lang.ResultSet.ArraySet(ex2.getXdr());	//horizontal
...
if( options.get("affiche_total_colonne")!=null){
		...
		buf.append("<br><br>");
		   ...
		   for(int j2=0;j2 < str_v3.length;j2++){
			...
			//	buf2.append("V1 :"+y+")"+arV.value(vertical2[k])+"<br>");
				...
				arV.moveFirst();
				while(arV.next()){
					if(!prec.equals(arV.value(vertical2[k]))){
						concat_valeur_distinct(rowspan,vertical2[k],""+y,sepa3);
					}else{
		...
		//			buf2.append("V2 :"+y+")"+arV.value(vertical2[k])+"<br>");
			...
			//		buf2.append("H1 :"+y+")"+arH.value(horizontal2[k])+"<br>");
					...
					y++;
					while(arH.next()){
						if(!prec.equals(arH.value(horizontal2[k]))){
							concat_valeur_distinct(colspan,horizontal2[k],""+y,sepa3);
						}else{
							if( th_present(horizontal2,k,y,colspan,sepa3)==0){
								concat_valeur_distinct(colspan,vertical2[k],""+y,sepa3);
							}
						}
		...
		buf.append("<center>Pas de résultat.</center>");
	}
...
return( buf2.toString() + buf.toString() );
		...
		return("<br><center>Erreur sur la requête.</center><br>");	
	}
