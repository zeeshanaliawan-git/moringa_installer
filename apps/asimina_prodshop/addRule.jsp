		...
		if(isOk)
		{
			...
			if(updateRule.equals("0"))
			{
				...
				qry = " insert into rules (start_proc, start_phase, errCode, next_proc, next_phase, action, priorite, nstate, rdv, type) "+
					" values ("+escape.cote(sourceProcess)+","+escape.cote(sourcePhase)+","+escape.cote(errCode)+","+escape.cote(targetProcess)+","+escape.cote(targetPhase)+","+escape.cote(ruleAction)+","+escape.cote(priorite+"")+","+escape.cote(nstate)+","+escape.cote(connRdv)+","+escape.cote(connType)+") ";
			}
			else
				qry = "update rules set rdv = "+escape.cote(connRdv)+", type = "+escape.cote(connType)+", nstate = "+escape.cote(nstate)+", action = "+escape.cote(ruleAction)+
						" where start_proc = "+escape.cote(sourceProcess)+" and start_phase = "+escape.cote(sourcePhase)+" and next_proc = "+escape.cote(targetProcess)+
						" and next_phase = "+escape.cote(targetPhase)+" and errCode = "+escape.cote(errCode);
