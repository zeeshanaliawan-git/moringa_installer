...
public class afficheFichier extends HttpServlet {
    ...
    throws ServletException, java.io.IOException
    {
        try {
            ...
            if(visu.equals("pdf")){
                ...
                doPdf(session,fileName,nomTdb,chemin);
            }
            if(visu.equals("xls")){
                ...
                fileName = new File(fic2);
                System.out.println("fic2="+fic2);
                doXls(session,fileName,nomTdb,chemin);
            }
    ...
    public void doPdf(HttpSession session,File fileName,String nomTdb,String chemin)
    throws Exception {
        int width = 750;
        int height = 450;
        try {
    ...
    public void doXls(HttpSession session,File fileName,String nomTdb,String chemin)
    throws Exception {
        try {
            ...
            for (int sht=0;sht<tabXls.length;sht++){
              ...
              /*  for(int j=0; j<tabXls.length ; j++){
                ...
                for(int j=0;j < tabXls[sht].Cols;j++){
                    cell = row.createCell((short) j);
                     cell.setCellValue(tabXls[sht].ColName[j]);
                }
