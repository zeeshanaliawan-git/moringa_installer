		...
		(fig instanceof draw2d.GizmoConnection && this.partialUpdate) )
	{
		...
		jQuery.ajax({
			...
			success: function(resp) {
				...
				jQuery('#'+____workflow.modalWindowId).dialog("option","title","New Rule");
				jQuery('#'+____workflow.modalWindowId).dialog("option","width","auto");
				jQuery('#'+____workflow.modalWindowId).dialog("option","height","auto");		
				jQuery('#'+____workflow.modalWindowId).html(resp);
				jQuery('#'+____workflow.modalWindowId).dialog('open');
			},
			error : function(resp) {
...
{	
	...
	jQuery('#'+this.modalWindowId).dialog("option","title","New Phase");
	jQuery('#'+this.modalWindowId).dialog("option","width","550px");
	jQuery('#'+this.modalWindowId).dialog("option","height","auto");
	jQuery('#'+this.modalWindowId).unbind("dialogclose");	   
	jQuery('#'+this.modalWindowId).html(html);
	jQuery('#'+this.modalWindowId).dialog('open');	
};
draw2d.GizmoWorkflow.prototype.OnNewConnection=function()
{
	...
	var prevConn = null;
	if(sPort.getConnections().size > 1 && updateRule != 1)
	{
	...
	var data = "connType="+connType+"&connRdv="+connRdv+"&nextestado="+nextestado+"&ruleAction="+ruleAction+"&updateRule="+updateRule+"&sourceProcess="+sPort.parentNode.group.process+"&sourcePhase="+sourcePhase+"&targetProcess="+conn.targetPort.parentNode.group.process+"&targetPhase="+targetPhase+"&errCode="+errCode+"&errColor="+errColor+"&errName="+errName+"&errType="+errType+"&errMsg="+errMsg;
	...
	jQuery.ajax({
		...
		success: function(resp) {
			...
			{		
				var cGrp = conn.group;
				jQuery('#'+____workflow.modalWindowId).dialog('close');
				if(!connectionsAlreadyForSourceToTarget)
				{
					//the connection which was added by user need to be added to workflow and then removed immediately otherwise it takes up a space in workflow 
					//and is also associated to the phase which should not be the case as we are going to add all connections ourselves now. 
		...
		if(fig.allowDelete)
		{
			...
			if(fig.group.errCode.size > 1)
			{
				jQuery.ajax({
					...
					success: function(resp) {
						resp = jQuery.trim(resp);
						jQuery('#'+____workflow.modalWindowId).dialog("option","title","Delete Rule");
						jQuery('#'+____workflow.modalWindowId).dialog("option","width","450");
						jQuery('#'+____workflow.modalWindowId).dialog("option","height","auto");		
						jQuery('#'+____workflow.modalWindowId).html(resp);
						jQuery('#'+____workflow.modalWindowId).dialog('open');
					},
...
draw2d.GizmoWorkflow.prototype.onOkNewPhase = function (currentProfile, pX, pY)
{
	...
	if(jQuery('#newPhaseName').val() == '')
	{
		jQuery('#newPhaseMsg').html("Enter Phase name");
		return;
	}
	if(jQuery('#priority').val() != '' && !IsNumeric(jQuery('#priority').val()))
	{
		jQuery('#newPhaseMsg').html("Priority should be numeric");
		return;
	}					
	
	if(jQuery('#newPhaseDisplayName').val() == '') jQuery('#newPhaseDisplayName').val(jQuery('#newPhaseName').val());
...
/*	var rulesVisibleTo = "";
	...
	var data = "topLeftX="+pX+"&topLeftY="+_pY+"&width="+defaultWidthFigure+"&height="+defaultHeightFigure+"&oprType="+oprType+"&isReverse="+isReverse+"&currentProfile="+currentProfile+"&visible="+vis+"&action=new&process="+____workflow.process+"&phase="+jQuery('#newPhaseName').val()+"&priority="+jQuery('#priority').val()+"&execute="+jQuery('#execute').val()+"&actors="+jQuery('#actors').val()+"&displayName="+jQuery('#newPhaseDisplayName').val();
	...
	jQuery.ajax({
		...
		success: function(resp) {
			...
			{	
				...
				var newFigGroup = new GizmoFigureGroup(jQuery('#newPhaseName').val(),jQuery('#actors').val(),jQuery('#priority').val(),jQuery('#execute').val(), vis, jQuery('#newPhaseDisplayName').val(),____workflow.process,____workflow.process);
			...
			else jQuery('#newPhaseMsg').html(resp.MESSAGE);
		},
		error : function(resp) {
			alert("Some error while communication with server. Try again or contact administrator");
		}				
...
draw2d.GizmoWorkflow.prototype.onExternalPhase=function(x,y)
{
	...
	html += "</table>";
	jQuery('#'+____workflow.modalWindowId).dialog("option","title","External Phase");
	jQuery('#'+____workflow.modalWindowId).dialog("option","width","350px");
	jQuery('#'+____workflow.modalWindowId).dialog("option","height","auto");
	jQuery('#'+____workflow.modalWindowId).unbind("dialogclose");	   
	jQuery('#'+____workflow.modalWindowId).html(html);
	jQuery('#'+____workflow.modalWindowId).dialog('open');
...
{
	var _pY = parseFloat(pY) - parseFloat(____workflow.deltaY);
	jQuery("#externalPhaseMsg").html("");
	if(jQuery("#otherProcessPhase").val() == '#')
	{	
		jQuery("#externalPhaseMsg").html("Select Phase");
		return;
	}
	jQuery.ajax({
		url: 'addExternalPhase.jsp',
		type: 'POST',
		data: {topLeftX: pX, topLeftY: _pY, width: defaultWidthFigure, height: defaultHeightFigure, process: jQuery('#process').val(), targetProcess: jQuery('#otherProcess').val(), targetPhase: jQuery('#otherProcessPhase').val() },
		dataType: 'json',
		success: function(resp) {
			if(resp.RESPONSE_TYPE == "SUCCESS")
			{
				...
				var groupObj = new GizmoFigureGroup(jQuery('#otherProcessPhase').val(),'','','','',displayName,jQuery('#otherProcess').val(), ____workflow.process);
				...
				newNode.setContent('<div style="background-color:gray; color:white; font-weight: bold"> Process : '+jQuery('#otherProcess').val()+'</div>');
				newNode.footer.style.backgroundColor="white";
			...
			{
				jQuery("#externalPhaseMsg").html(resp.MESSAGE);						
			}
		}
	});						
};
