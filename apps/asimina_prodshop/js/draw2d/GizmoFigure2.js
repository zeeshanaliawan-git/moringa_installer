...
function onEditPhase(proc, oldPhase)
{
	...
	*/
	...
	var data = "oprType="+oprType+"&isReverse="+isReverse+"&visible="+vis+"&action=edit&process="+proc+"&oldPhase="+oldPhase+"&phase="+oldjquery('#editPhaseName').val()+"&priority="+oldjquery('#editPriority').val()+"&execute="+oldjquery('#editExecute').val()+"&color="+oldjquery('#editColor').val()+"&actors="+oldjquery('#editActors').val()+"&displayName="+oldjquery('#editPhaseDisplayName').val();
	oldjquery.ajax({
		url: 'addNewPhase.jsp',
		type: 'POST',
		data: data,
		...
		success: function(resp) {
			...
			{	
				editPhase.group.setFigureName(oldjquery('#editPhaseName').val());
				editPhase.group.setDisplayName(oldjquery('#editPhaseDisplayName').val());
				editPhase.group.setActors(oldjquery('#editActors').val());
				editPhase.group.setPriority(oldjquery('#editPriority').val());
				editPhase.group.setExecute(oldjquery('#editExecute').val());
				editPhase.group.setColor(oldjquery('#editColor').val());
			...
			{
				oldjquery('#editPhaseMsg').html(resp.MESSAGE);
			}
		},
		error : function(resp) {
			alert("Some error while communication with server. Try again or contact administrator");
...
//		this.html.style.top = this.y;
		...
		oldjquery('#'+this.outputPort.workflow.modalWindowId).dialog("option","title","Copy/Move Phase");
		oldjquery('#'+this.outputPort.workflow.modalWindowId).dialog("option","width","auto");
		oldjquery('#'+this.outputPort.workflow.modalWindowId).dialog("option","height","auto");		
		oldjquery('#'+this.outputPort.workflow.modalWindowId).html(h);
		oldjquery('#'+this.outputPort.workflow.modalWindowId).dialog('open');			
		return;
	}
