...
<%
	...
	if(request.getParameter("saveConfiguration") != null)
	{
		...
		{
			/*if(!emailto.equals("") && emailto.startsWith("["))
			{
				String temp = emailto;
				temp = temp.replace("[","");
				temp = temp.replace("]","");
				Set rs = Etn.execute("select "+temp+" from customer limit 1");
				if(rs == null) out.write("{\"MESSAGE\":\"Not a valid column name given in Email\"}");				
			}
			else if(!emailto.equals(""))
			{				
				if(!isValidEmailAddress(emailto)) out.write("{\"MESSAGE\":\"Not a valid Email address given\"}");				
			}

			if(!whereclause.equals(""))
			{
				Set rs = Etn.execute("select customerid from customer where "+whereclause+" limit 1");
				if(rs == null) out.write("{\"MESSAGE\":\"Not a valid condition given\"}");				
			}*/
