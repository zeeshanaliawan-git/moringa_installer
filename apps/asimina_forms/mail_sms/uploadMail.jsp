...
<%
	...
	try
	{
		...
		if(uploadFile != null)
		{
			...
			if("text/plain".equalsIgnoreCase(_type) == false)
			{
		...
		if(mailId.equals("0"))
		{
			Set rs = Etn.execute("select * from mails where sujet = "+com.etn.sql.escape.cote(subject));
			if(rs.rs.Rows > 0)
			{
				s = 2;
				isError = true;
			...
			{
				int row = Etn.executeCmd("insert into mails (sujet, seq ) values ("+com.etn.sql.escape.cote(subject)+",'0') ");
				mailId = "" + row;
			}
		}
