...
<%
	...
	if(rsAction.rs.Rows == 0)
	{
		...
		rows = Etn.executeCmd("insert into has_action(start_proc, start_phase, cle, action) values ("+escape.cote(sourceProcess)+","+escape.cote(sourcePhase)+","+escape.cote(cle)+","+escape.cote(ruleAction)+") ");//change
	}
	else
	{
		rsAction.next();
		cle = rsAction.value("cle");			
		rows = Etn.executeCmd("update has_action set action = "+escape.cote(ruleAction)+" where cle = "+escape.cote(cle)+" and start_proc = "+escape.cote(sourceProcess)+" and start_phase = "+escape.cote(sourcePhase)+" ");//change 
	}
	if(rows > 0)
	{
		...
		rsAction = Etn.execute("select * from has_action a where a.cle = "+escape.cote(cle)+" and a.start_proc = "+escape.cote(sourceProcess)+" and a.start_phase = "+escape.cote(sourcePhase)+" ");//change 
		if(rsAction.next() && parseNull(rsAction.value("action")).length() == 0)
		{
			Etn.execute("delete from has_action where cle = "+escape.cote(cle)+" and start_proc = "+escape.cote(sourceProcess)+" and start_phase = "+escape.cote(sourcePhase)+" ");//change 
		}
