...
<%
	String tableId = parseNull(request.getParameter("tid"));

	String form_id = parseNull(request.getParameter("form_id"));

	String rowid = tableId;
    ...
    String calendar_client_key = tableId;

        if(calendar_client_key.length() > 0){
            rowid = calendar_client_key;
            Set rsOrder = Etn.execute("select * from post_work where status IN (0,9) AND phase NOT IN ('Cancel','Cancel30') AND form_table_name = "+escape.cote(process)+" AND client_key = "+escape.cote(calendar_client_key));
            if(rsOrder.next()){
                ...
                if(currentPhase.equals(nextPhase)){
                    out.write("samePhase");
                    Etn.executeCmd("updade post_work SET priority=NOW() where id="+post_work_id);
...
<% if(rs.rs.Rows == 1) { 
	...
	%>
	<script>
		onOkChangePhase('<%=rowid%>', '<%=process%>', '<%=currentPhase%>', '<%=nextProcess%>','<%=nextPhase%>', '<%=fromDialog%>','<%=post_work_id%>','<%=rs.value("errCode")%>')
	</script>
<% } else { %>
<style>
...
<div>
	<table id='errTbl' cellspacing='0' cellpadding='1' border='0' width="100%">
		...
		<% while(rs.next()) { %>
			<tr>
				<td align="center"><input type='radio' name='changePhaseErrCode' value='<%=rs.value("errCode")%>' /></td>				
...
<div>
	<div onclick="javascript:onOkChangePhase('<%=rowid%>', '<%=process%>', '<%=currentPhase%>', '<%=nextProcess%>','<%=nextPhase%>', '<%=fromDialog%>','<%=post_work_id%>')" style="width:50px; padding-left:1px; margin-top:2px; margin-bottom:1px; cursor:pointer; background-color:<%=ORANGE%>; color:white; border-right:1px solid #bc3a00; border-bottom:1px solid #bc3a00">Ok</div>
</div>
<% } %>