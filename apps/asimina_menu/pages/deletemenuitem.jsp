...
<%
	...
	Set rs = Etn.execute("select * from menu_items where menu_id = "+escape.cote(menuid)+" and id = " + escape.cote(menuitemid));
	if(rs.next())
	{
		if("VConnectLive".equalsIgnoreCase(rs.value("name")) && "-30".equals(rs.value("menu_item_id")))
		{
			out.write("{\"response\":\"error\",\"msg\":\"This item cannot be deleted\"}");
			return;
		}
	}		

	String q = "delete from menu_items where menu_id = "+escape.cote(menuid)+" and id = " + escape.cote(menuitemid);
	...
	else out.write("{\"response\":\"error\",\"msg\":\"Error deleting menu item\"}");
%>
