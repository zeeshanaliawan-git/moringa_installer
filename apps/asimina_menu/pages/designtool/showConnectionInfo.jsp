...
%>
<%@ page import="com.etn.lang.ResultSet.Set, com.etn.sql.escape, com.etn.util.CommonHelper"%>
...
%>
			<tr>
				...
				<%if(!forDelete.equals("1") && allowEdit.equals("1")) {%>
				<td>&nbsp;<a style='color:black' href="javascript:onEditRule('<%=CommonHelper.escapeCoteValue(sourceProcess)%>','<%=CommonHelper.escapeCoteValue(sourcePhase)%>','<%=CommonHelper.escapeCoteValue(targetProcess)%>','<%=CommonHelper.escapeCoteValue(targetPhase)%>','<%=rs.value("errCode")%>',1)">Edit</a>&nbsp;</td>
				<%}
				else if(!forDelete.equals("1") && !allowEdit.equals("1")) {%>
				<td>&nbsp;<a style='color:black' href="javascript:onEditRule('<%=CommonHelper.escapeCoteValue(sourceProcess)%>','<%=CommonHelper.escapeCoteValue(sourcePhase)%>','<%=CommonHelper.escapeCoteValue(targetProcess)%>','<%=CommonHelper.escapeCoteValue(targetPhase)%>','<%=rs.value("errCode")%>',0)">View</a>&nbsp;</td>
				<%}%>
			</tr>
<% } %>
		</table>
						...
						success: function(resp) {
							...
							for(var i=0; i<workflow.lines.size; i++)
							{
								if(workflow.lines.get(i) instanceof draw2d.GizmoConnection &&
									workflow.lines.get(i).sourcePort.parentNode.group.figName == '<%=CommonHelper.escapeCoteValue(sourcePhase)%>' &&
									workflow.lines.get(i).sourcePort.parentNode.group.process == '<%=CommonHelper.escapeCoteValue(sourceProcess)%>' &&
									workflow.lines.get(i).targetPort.parentNode.group.figName == '<%=CommonHelper.escapeCoteValue(targetPhase)%>' &&
									workflow.lines.get(i).targetPort.parentNode.group.process == '<%=CommonHelper.escapeCoteValue(targetProcess)%>' )
								{
