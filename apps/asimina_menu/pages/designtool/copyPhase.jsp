...
<%
	...
	String qry = "select * from phases where process = '"+replaceQoute(process)+"' and phase = '"+replaceQoute(phase)+"'";
	...
	else
	{
		...
		qry +=   " WHERE process = '"+replaceQoute(process)+"' AND phase = '"+replaceQoute(phase)+"' ;";
		...
		{		
			Set rs1 = Etn.execute("select * from coordinates where process = '"+replaceQoute(process)+"' and phase = '"+replaceQoute(phase)+"' and profile = '"+replaceQoute(profile)+"' ");
			if(rs1.rs.Rows == 0)
			{
				Etn.executeCmd("insert into coordinates (process, profile, phase, topLeftX, topLeftY, width, height) values ('"+replaceQoute(process)+"','"+replaceQoute(profile)+"','"+replaceQoute(phase)+"',"+escape.cote(x)+","+escape.cote(y)+","+escape.cote(width)+","+escape.cote(height)+")");				
			}
			else
			{
				Etn.executeCmd ("update coordinates set topLeftX = "+escape.cote(x)+", topLeftY = "+escape.cote(y)+", width = "+escape.cote(width)+", height = "+escape.cote(height)+" where process = '"+replaceQoute(process)+"' and phase = '"+replaceQoute(phase)+"' and profile = '"+replaceQoute(profile)+"' ");
			}
