...
<% if(callAjax.equalsIgnoreCase("true") || partialUpdate.equalsIgnoreCase("true")) { %>
	<div style='text-align:center; margin-top:3px'><input type='button' id='newPhaseBtn' value='Ok' onclick=onEditPhase('<%=CommonHelper.escapeCoteValue(process)%>','<%=CommonHelper.escapeCoteValue(phase)%>')></div>
<% } %>
</div>
