...
<%
	...
	if(isUpdate)
	{
		...
		Set ruleRs = Etn.execute("select * from rules where start_proc='"+replaceQoute(sourceProcess)+"' and start_phase = '"+replaceQoute(sourcePhase)+"' and next_proc='"+replaceQoute(targetProcess)+"' and next_phase = '"+replaceQoute(targetPhase)+"' and errCode = '"+replaceQoute(errCode)+"' ");
		ruleRs.next();
...
<% } %>			   
<script>
	jQuery('#colorPicker').ColorPicker({onSubmit: function(hsb, hex, rgb, el) { jQuery(el).val(hex); jQuery(el).ColorPickerHide(); }});
	
	function setErrInfo(errCode, errNom, errType, errColor, errMsg)
	{
		<% if(!isUpdate) { %>
			jQuery('#newConnErrCode').val(errCode);
		<% } %>
		jQuery('#newConnErrName').val(errNom);
		jQuery('#newConnErrMsg').val(errMsg);
		jQuery('#newConnErrType').val(errType);
		if(errColor.indexOf("#") > -1)
			errColor = errColor.substr(1, errColor.length);
		jQuery('#colorPicker').val(errColor);
	}
