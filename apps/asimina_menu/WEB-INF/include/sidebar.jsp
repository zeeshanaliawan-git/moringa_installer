...
%>
		...
		<img class='c-sidebar-brand-minimized' src="<%=request.getContextPath()%>/img/logo.png" alt="" style="height:20px;width:auto;vertical-align: bottom;">
	</div>
...
<%
										...
										<li class="c-sidebar-nav-item" style="background-color: black; user-select: auto;">
											...
											onclick="$.ajax({ url : '<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/updatesiteselection.jsp', type: 'POST', data: { id: $(this).prop('id')}, dataType : 'json', success : function(json) { if(json.resp == 'error') { alert('Error updating site selection'); window.location = '<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/gestion.jsp'; } if(json.resp =='success'){ window.location = '<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/gestion.jsp'; } }, error : function() { window.location = '<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/gestion.jsp'; } });"
									...
									<% if(!useAssignedSites) { %> 
									<li class="c-sidebar-nav-item" style="background-color: black; user-select: auto;">
										<a class="c-sidebar-nav-link" href="<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/newsite.jsp">+ Add Site
										</a>
										</li> 
									<%}%>
						</ul>
					</li>
					<li class="c-sidebar-nav-item c-searchfield" style="user-select: auto;">
				...
				%>
							...
							%>
								<li class="c-sidebar-nav-item"><a class="c-sidebar-nav-link" href="<%=____u%>" <%=____target%>><i class="c-sidebar-nav-icon" data-feather="<%=____rs1.value("icon")%>"></i>
									<%=____rs1.value("name")%>
									<%if(parseNull(____rs1.value("menu_badge")).length() > 0){%>
									<span class="badge badge-success"><%=parseNull(____rs1.value("menu_badge"))%></span>
									<%}%>
								</a></li>
							<% } %>
							</ul>
				...
				<%	  	}
					if(____rsp.value("_parent").equals("Dashboard")){
						...
						<li class="c-sidebar-nav-item c-sidebar-nav-dropdown c-sidebar-nav-item-alert mt-auto c-sidebar-user-item" style="user-select: auto;">
							<a class="c-sidebar-nav-link c-sidebar-nav-dropdown-toggle c-sidebar-user-link" href="#" style="background-color: black; user-select: auto;">
        ...
        <script type="text/javascript">
            $('#global_search_text').keyup(function(event) {
                if(event.which === 13){
                    $('body').append('<form method="post" id="frm_global_search" action="<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/search.jsp" ><input type="hidden" name="global_search" value="'+$('#global_search_text ').val().trim()+'"/><input type="hidden" name="status" value="All"/></form>');
			...
			callback: function (result) {
				...
				if (result) {
					...
					.always(function (jqXHR) {
						...
						if(jqXHR.status == 401)
						{
							window.location.href = "<%=com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT")%>/admin/gestion.jsp?_tm=<%=System.currentTimeMillis()%>";
						}
					});
				}
			}
