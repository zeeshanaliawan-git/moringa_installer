  ...
  <%-- <script src="<%=request.getContextPath()%>/js/support_proxy.jsp?__v=<%=com.etn.beans.app.GlobalParm.getParm("CSS_JS_VERSION")%>p"></script> --%>
...
<%
	...
	Set __rssite = Etn.execute("select * from sites where is_active = 1 order by name " );
	if(useAssignedSites)
	{
		__rssite = Etn.execute("select s.* from sites s, person_sites ps where s.is_active = 1 and ps.site_id = s.id and ps.person_id = "+escape.cote(""+Etn.getId())+" order by name " );
	}

	String __selectedsite = "";
	String __selectedsitename = "Select Site";
	Set __rssessioins = Etn.execute("select coalesce(selected_site_id,'') as selected_site_id from " + com.etn.beans.app.GlobalParm.getParm("COMMONS_DB") + ".user_sessions where menu_session_id = " + escape.cote(""+session.getId()));
	...
	{
		com.etn.lang.ResultSet.Set ___rss = Etn.execute("select * from sites where is_active = 1 and id = " +com.etn.sql.escape.cote(___sbSiteId) );
		if(___rss.next())
		{
