...
public class SiteHelper {
    ...
    public static int insertSiteLang(Contexte Etn, String siteId, String langueId) {
        int id = Etn.executeCmd("INSERT INTO " + COMMONS_DB + ".sites_langs values(" + escape.cote(siteId) + ","
                + escape.cote(langueId) + ")");
        return id;
    }

    public static void removeSiteLangs(Contexte Etn, String siteId, String langueId) {
        Etn.execute("DELETE FROM " + COMMONS_DB + ".sites_langs WHERE langue_id=" + escape.cote(langueId)
                + " AND site_id=" + escape.cote(siteId));
    }

	public static String getSiteId(javax.servlet.http.HttpSession session)
	{
       	return com.etn.util.CommonHelper.parseNull(session.getAttribute("SELECTED_SITE_ID"));
	}
}
