...
public class Authenticate implements Filter
{
	...
	public void doFilter(ServletRequest _request, ServletResponse _response, FilterChain chain) throws IOException, ServletException
	{
		...
		if(Etn.getId() == 0)
		{
			...
			Logger.info("Menu::Authenticate","Webapp token : "+webappToken);				
			if(webappToken.length() > 0)
			{
				...
				if(rsW.next())
				{
					...
					Logger.info("Menu::Authenticate","accessid : "+accessid);
					String catalogsessionid = rsW.value("catalog_session_id");
					Logger.info("Menu::Authenticate","catalogsessionid : "+catalogsessionid);
					...
					if(rs.next())
					{
						...
						if(Integer.parseInt(timediff) > (Integer.parseInt(timeout) * 60))
						{
							...
							Logger.info("Menu::Authenticate", "Session timeout");
						}
						else
						{
							Etn.setContexte(rs.value("name"),rs.value("pass"));
							Etn.executeCmd("insert into "+GlobalParm.getParm("COMMONS_DB")+".user_sessions (catalog_session_id, menu_session_id) values ("+escape.cote(catalogsessionid)+","+escape.cote(""+request.getSession().getId())+") on duplicate key update last_updated_on = now(), menu_session_id = "+escape.cote(""+request.getSession().getId()));
							gotogestion = false;
						}
					}
				...
				{
					Logger.info("Menu::Authenticate", "Webapp token is expired");
				}
			}

			Logger.info("Menu::Authenticate","gotogestion : "+gotogestion);
			...
			if(gotogestion)
			{
				...
				String redirectUrl = GlobalParm.getParm("GOTO_MENU_APP_URL");
	...
	if(Etn.getId() > 0)
	{
		...
		Logger.info("Menu::Authenticate","Etn.getId:"+Etn.getId()+" canContinue:"+canContinue+" IP:"+ip);
