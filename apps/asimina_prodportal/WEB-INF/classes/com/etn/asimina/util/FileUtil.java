...
public class FileUtil {
    ...
    public static File getFile(String directory, String name) {
		if(PortalHelper.parseNull(directory).length() == 0) {
			Logger.error("FileUtil.java", "directory is empty");
			return null;
		}
		if(PortalHelper.parseNull(name).length() == 0) {
    ...
    public static File getFile(String path)  {
		if(PortalHelper.parseNull(path).length() == 0) {
	...
	public static String getMimeTypeExtension(String fileName)  {
		if(PortalHelper.parseNull(fileName).length() == 0) {
			Logger.error("FileUtil.java", "file path is empty");
			return null;
		}
    ...
    public static FileInputStream getFileInputStream(String filepath)  {
		if(PortalHelper.parseNull(filepath).length() == 0) {
	...
	public static String sanitizePath(String path) {
		return PortalHelper.parseNull(path).replaceAll("\\.\\.(/|\\\\)", "").replaceAll("\\\\", "");
    }
}