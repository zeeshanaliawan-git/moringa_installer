...
<%@ page import="com.etn.asimina.util.*, com.etn.asimina.cart.*,com.etn.asimina.beans.Cart" %>
...
<%
	...
	Set rsM = Etn.execute("select * from site_menus where menu_uuid = "+escape.cote(___muuid));
	String siteId = "";
	if(rsM.next()) siteId = rsM.value("site_id");

	String sessionToken = parseNull(request.getParameter("session_token"));
	String dbSessionToken = CartHelper.getSessionToken(Etn, session_id, siteId);	
    Cart cart = CartHelper.loadCart(Etn,request,session_id,siteId,___muuid,true);
    ...
    String status = CartHelper.getKycStatus(Etn,cart.getProperty("id"));

	//reset session token in db for next screen
    String uuidSessionToken = CartHelper.setSessionToken(Etn, session_id, siteId);
...
<article class="container-lg TunnelNotice">
    <div class="TunnelNotice-titleContainer">
        ...
        <h3 class="TunnelNotice-title"><%= (!status.equalsIgnoreCase("expired"))? "Erreur de vérification KYC": "Session expirée" %></h3>
    </div>
    ...
    <div class="TunnelNotice-bodyContainer">
        ...
        <div class="TunnelNotice-contentContainer">
            <p class="TunnelNotice-textBold">
                <%= (!status.equalsIgnoreCase("expired"))? "Nous sommes désolés, mais nous n'avons pas pu vérifier votre identité pour le moment. Veuillez vous assurer que les informations fournies correspondent aux documents soumis. Si vous pensez qu'il s'agit d'une erreur, veuillez contacter le service client pour obtenir de l'aide.": "Veuillez refaire la vérification KYC." %>
            </p>
            <a href="javascript:gotoRecap();" class="btn btn-secondary isFullWidthMobile TunnelNotice-button">Retour à la vérification KYC</a>
        </div>
    </div>
</article>
<form name="myform" id='myform' action="kyc.jsp" method="post" class="FormValidator kycform row justify-content-between"
                  data-messages='{"required":"<%=libelle_msg(Etn, request, "Ce champ est requis.")%>", "regex":"<%=libelle_msg(Etn, request, "Ce champ est non valide.")%>", "confirm":"<%=libelle_msg(Etn, request, "Veuillez saisir deux fois la même valeur.")%>"}'>
