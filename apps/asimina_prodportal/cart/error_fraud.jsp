...
<%
  String _jsp = "/cart/default_error_fraud.jsp";
  boolean isEshopJsp=parseNull(com.etn.beans.app.GlobalParm.getParm("ESHOP_FRAUD_JSP")).length() > 0;
  if(isEshopJsp) _jsp = parseNull(com.etn.beans.app.GlobalParm.getParm("ESHOP_FRAUD_JSP"));

  String ___muuid = CartHelper.getCartMenuUuid(request);

  if(parseNull(___muuid).length()>0){
    Set rs=Etn.execute("SELECT sd.fraud_page_url,m.id FROM site_menus m "+
    ...
    if(rs!=null && rs.next()){
      if(parseNull(rs.value("fraud_page_url")).length()>0){
        response.sendRedirect(com.etn.asimina.util.PortalHelper.getMenuPath(Etn, parseNull(rs.value("id")))+parseNull(rs.value("fraud_page_url"))); 
        return; 
      }
    }
  }
