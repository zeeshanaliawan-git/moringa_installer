<div class="TunnelBreadcrumb">
    <div class="container-lg">
        <div class="TunnelBreadcrumb-wrapper">
            ...
            <div class="TunnelBreadcrumb-step TunnelBreadcrumb-step-informations <%=(stepNumber==1?"isCurrent isVisited":(stepNumber>1?"isVisited":""))%>" <%=(stepNumber>1?"onclick=\"gotoStep('checkout.jsp')\" style=\"cursor:pointer\"":"")%>>
                <div class="TunnelBreadcrumb-circle">
                    <span class="TunnelBreadcrumb-svg"
                                  data-svg="/src/assets/icons/icon-my-account.svg"></span>
                </div>
            </div>
            <div class="TunnelBreadcrumb-step TunnelBreadcrumb-step-delivery <%=(stepNumber==2?"isCurrent isVisited":(stepNumber>2?"isVisited":""))%> " <%=(stepNumber>2?"onclick=\"gotoStep('deliv.jsp')\" style=\"cursor:pointer\"":"")%>>
                <div class="TunnelBreadcrumb-circle">
                    <span class="TunnelBreadcrumb-svg"
                                  data-svg="/src/assets/icons/icon-fast-delivery.svg"></span>
                </div>
            </div>
            <div class="TunnelBreadcrumb-step TunnelBreadcrumb-step-payment <%=(stepNumber==3?"isCurrent isVisited":(stepNumber>3?"isVisited":""))%> " <%=(stepNumber>3?"onclick=\"gotoStep('payment.jsp')\" style=\"cursor:pointer\"":"")%>>
                <div class="TunnelBreadcrumb-circle">
                   <span class="TunnelBreadcrumb-svg"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <form class="ModalValidator-form"
                          data-messages='{"required":"<%=escapeCoteValue(libelle_msg(Etn, request, "Ce champ est requis."))%>", "regex":"<%=escapeCoteValue(libelle_msg(Etn, request, "Ce champ est non valide."))%>", "confirm":"<%=escapeCoteValue(libelle_msg(Etn, request, "Veuillez saisir deux fois la mï¿½me valeur."))%>"}'>

                        <div class="TunnelExitModal-title h2">
                            <span data-svg="/src/assets/icons/icon-info.svg"></span>
                            <p><strong><%=libelle_msg(Etn, request, "Vous êtes sur le point de quitter votre commande.")%></strong></p>
                        </div>
...
//                }
        ...
        if(step=="cart.jsp") $('<form method="GET" action="'+step+'"><input type="hidden" name="muid" value="'+______muid+'" ></form>').appendTo('body').submit();
        else $('<form method="POST" action="'+step+'"><input type="hidden" name="muid" value="'+______muid+'" ><input type="hidden" name="session_token" value="'+$('input[name=session_token]').val()+'" ></form>').appendTo('body').submit();
    }
