...
( function() {
	CKEDITOR.dialog.add( 'link', function( editor ) {
		...
		var customUrlValidate = null;
		var initUrlFieldInput = function(input, onSelectCallback){
			if(!window.etn || !window.etn.initUrlGenerator || !window.URL_GEN_AJAX_URL){
				return false;
			}
			
			//to get language ID we might have to implement different cases here as in ckeditor we do not know language
			//so depending on how lang tabs are drawn we change here as well
			let _langId = "0";			
			[].forEach.call(document.querySelectorAll('[data-language-id]'), function(_obj) {
				if(_obj.classList.contains("active"))
				{
					_langId = _obj.getAttribute("data-language-id");
				}
			});
			if(_langId == "0")
			{
				[].forEach.call(document.querySelectorAll('[data-lang-id]'), function(_obj) {
					if(_obj.classList.contains("active"))
					{
						_langId = _obj.getAttribute("data-lang-id");
					}
				});
			}
			console.log("ckeditor-link.js::_langId="+_langId);
			...
			var urlGenOpts = {
				...
				template : null,
				autoCompleteAppendTo : jQuery(input).parent(),
			};

			var gen = etn.initUrlGenerator(input, window.URL_GEN_AJAX_URL, urlGenOpts);
			gen.setOnSelectCallback(onSelectCallback)
			jQuery(input).data('url-generator',gen);

			customUrlValidate = function(othis){
				var dialog = othis.getDialog();
				if ( dialog.getContentElement( 'info', 'linkType' )
						&& dialog.getValueOf( 'info', 'linkType' ) != 'url' ){
					return true;
				}

				var inputElement = jQuery(othis.getInputElement().$);
				inputElement.removeClass('border-danger');

				if(inputElement.hasClass('is-invalid')){
					inputElement.addClass('class_name')('border-danger');
				}
						...
						},
						{
							...
							setup: function( data ) {
								...
								var onSelect = function(){
									var protocolField = oThis.getDialog().getContentElement( 'info', 'protocol' );
									protocolField.setValue('');
									oThis.onChange();
								};								
								initUrlFieldInput(jQuery(this.getInputElement().$), onSelect);
								this.allowOnChange = true;
