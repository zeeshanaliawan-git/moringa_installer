...
public class Authenticate implements Filter
{
	...
	public void doFilter(ServletRequest _request, ServletResponse _response, FilterChain chain) throws IOException, ServletException
	{
		...
		Logger.info("Pages::Authenticate","ip " + ip + " " + request.getRequestURI());
		
		if(ip.equals("127.0.0.1") && request.getRequestURI().startsWith(request.getContextPath()+"/admin/pages/"))
		{
			Logger.info("Pages::Authenticate", "Internal call to generated page so we not check auth");
			chain.doFilter(request, response);
		}
		else
		{
			System.out.println("Check pages auth");
			if(Etn.getId() == 0)
			{
				...
				Logger.info("Pages::Authenticate","Webapp token : "+webappToken);				
				if(webappToken.length() > 0)
				{
					...
					if(rsW.next())
					{
						...
						Logger.info("Pages::Authenticate","accessid : "+accessid);
						String catalogsessionid = rsW.value("catalog_session_id");
						Logger.info("Pages::Authenticate","catalogsessionid : "+catalogsessionid);
						...
						Set rs = Etn.execute("select *,  TIMESTAMPDIFF(SECOND, access_time, now()) as _diff from login where access_id = " +  com.etn.sql.escape.cote(accessid)  );
						if(rs.next())
						{
							...
							if(Integer.parseInt(timediff) > (Integer.parseInt(timeout) * 60))
							{
								...
								Logger.info("Pages::Authenticate", "Session timeout");
							}
							else
							{
								Etn.setContexte(rs.value("name"),rs.value("pass"));
								Etn.executeCmd("insert into "+com.etn.beans.app.GlobalParm.getParm("COMMONS_DB")+".user_sessions (catalog_session_id, pages_session_id) values ("+com.etn.sql.escape.cote(catalogsessionid)+","+com.etn.sql.escape.cote(""+request.getSession().getId())+") on duplicate key update last_updated_on = now(), pages_session_id = "+com.etn.sql.escape.cote(""+request.getSession().getId()));
								gotogestion = false;
							}
						}
					...
					{
						Logger.info("Pages::Authenticate", "Webapp token is expired");
					}
				}

				Logger.info("Pages::Authenticate","gotogestion : "+gotogestion);
				if(gotogestion)
				{
					//update by Ali Adnan
					...
					String redirectUrl = com.etn.beans.app.GlobalParm.getParm("GESTION_URL");
					redirectUrl += redirectUrl.indexOf("?") > 0  ? "&" : "?";
					redirectUrl += "_url="+ URLEncoder.encode(requestedUrl,"UTF-8");
					// System.out.println("PAGES:: URL :" + redirectUrl);
			...
			if(Etn.getId() > 0)
			{
				...
				Logger.info("Pages::Authenticate.java","Etn.getId:"+Etn.getId()+" canContinue:"+canContinue+" IP:"+ip);
			...
			{
				String req_login_1= "Select pr.is_webmaster, pr.profil, l.name,l.pid, p.first_name, p.Last_name,pp.profil_id, l.access_id from login l left join person p on p.person_id=l.pid left join profilperson pp on pp.person_id=l.pid left join profil pr on pr.profil_id = pp.profil_id where l.pid="+ escape.cote(""+Etn.getId());//change
				...
				if(res_1.next())
				{
					...
					request.getSession().setAttribute("IS_WEBMASTER",res_1.value("is_webmaster"));
				}
			}
			...
			int _r = Etn.executeCmd("update login set access_time = now() where pid =  " + Etn.getId() + " and access_id = " + com.etn.sql.escape.cote(sAccessId));
