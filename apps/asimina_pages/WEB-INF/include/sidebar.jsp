<%
	...
	Set __rssessioins = Etn.execute("select coalesce(selected_site_id,'') as selected_site_id from " + com.etn.beans.app.GlobalParm.getParm("COMMONS_DB") + ".user_sessions where pages_session_id = " + escape.cote(""+session.getId()));
    ...
    String __CATALOG_ROOT = com.etn.beans.app.GlobalParm.getParm("CATALOG_ROOT");
	...
	Set ____rsp = Etn.execute("select case coalesce(parent,'') when '' then name else parent end as _parent, case coalesce(icon,'') when '' then icon else parent_icon end as _icon, " +
			" case coalesce(parent,'') when '' then 0 else 1 end as _isparent, min(rang) as rang from page where 1=1 "+whereClause+
			" group by _parent order by rang, _parent");
...
%>
    <div id="sidebar" class="c-sidebar c-sidebar-dark c-sidebar-fixed c-sidebar-lg-show">
		<div class="c-sidebar-brand d-lg-down-none" style="user-select: auto;">
			<div class="c-sidebar-brand-full" style="user-select: auto;">
				<img src="<%=__CATALOG_ROOT%>/img/logo.png" alt="" style="height: 25px; width: auto; vertical-align: bottom; user-select: auto;">
			...
			<img class='c-sidebar-brand-minimized' src="<%=__CATALOG_ROOT%>/img/logo.png" alt="" style="height:20px;width:auto;vertical-align: bottom;">
		</div>
				...
				<ul class="c-sidebar-nav-dropdown-items" style="user-select: auto;">
					...
					%>
							<li class="c-sidebar-nav-item" style="background-color: black; user-select: auto;">
								...
								onclick="$.ajax({ url : '<%=__CATALOG_ROOT%>/admin/updatesiteselection.jsp', type: 'POST', data: { id: $(this).prop('id')}, dataType : 'json', success : function(json) { if(json.resp == 'error') { alert('Error updating site selection'); window.location = '<%=__CATALOG_ROOT%>/admin/gestion.jsp'; } if(json.resp =='success'){ window.location = '<%=__CATALOG_ROOT%>/admin/gestion.jsp'; } }, error : function() { window.location = '<%=__CATALOG_ROOT%>/admin/gestion.jsp'; } });"
					...
					%> 
							<li class="c-sidebar-nav-item" style="background-color: black; user-select: auto;">
								<a class="c-sidebar-nav-link" href="<%=__CATALOG_ROOT%>/admin/newsite.jsp">+ Add Site
								</a>
							</li> 
					<%
						}
		...
		<%			}
					if("0".equals(____rsp.value("_isparent"))){
						String ____q = "select * from page p where p.name = "+com.etn.sql.escape.cote(____rsp.value("_parent"))+
			...
			%>
						<li class="c-sidebar-nav-item c-sidebar-nav-dropdown">
							<a class="c-sidebar-nav-link c-sidebar-nav-dropdown-toggle" href="#">
								<i class="c-sidebar-nav-icon" data-feather="<%=____rsp.value("_icon")%>"></i> 
	...
	<script type="text/javascript">
		$('#global_search_text').keyup(function(event) {
			if(event.which === 13){
				$('body').append('<form method="post" id="frm_global_search" action="<%=__CATALOG_ROOT%>/admin/search.jsp" ><input type="hidden" name="global_search" value="'+$('#global_search_text ').val().trim()+'"/><input type="hidden" name="status" value="All"/></form>');
				...
				callback: function (result) {
					...
					if (result) {
						...
						.always(function (jqXHR) {
							...
							if(jqXHR.status == 401)
							{
								window.location.href = "<%=__CATALOG_ROOT%>/admin/gestion.jsp?_tm=<%=System.currentTimeMillis()%>";
							}
						});
					}
				}
		...
		function addToShortcut(pageName){
			$.ajax({
				url: '<%=request.getContextPath()%>/admin/pagesAjax.jsp', type: 'POST', dataType: 'json',
				data: {
