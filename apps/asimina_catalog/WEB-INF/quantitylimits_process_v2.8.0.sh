...
procname="quantitylimits"

mysql -u $dbuser -h $dbhost -P $dbport $cmdpass $DB_NAME << EOF
insert into rules (start_proc,start_phase,errCode,next_proc,next_phase,nextestado,action,priorite,tipo,rdv,nstate,type) values ('$procname','delete','0','$procname','deleted','0','remove:quantitylimits','0','0','And','0','Cancelled');
insert into rules (start_proc,start_phase,errCode,next_proc,next_phase,nextestado,action,priorite,tipo,rdv,nstate,type) values ('$procname','publish','0','$procname','published','0','publish:quantitylimits','0','0','And','0','Cancelled');
EOF
...
then
	...
	cle=${results[0]}
	echo "cle: $cle"
	mysql -u $dbuser -h $dbhost -P $dbport $cmdpass ${DB_NAME} -e "insert into has_action (start_proc,start_phase,cle,action) values ('$procname','delete','$cle','remove:quantitylimits');"
fi
...
then
	...
	cle=${results[0]}
	echo "cle: $cle"
	mysql -u $dbuser -h $dbhost -P $dbport $cmdpass ${DB_NAME} -e "insert into has_action (start_proc,start_phase,cle,action) values ('$procname','publish','$cle','publish:quantitylimits');"
fi
