...
%>
					...
					<li class="c-sidebar-nav-item c-sidebar-nav-dropdown" style="user-select: auto;">
						...
						<ul class="c-sidebar-nav-dropdown-items" style="user-select: auto;">
							<%
									...
									<% if(!useAssignedSites) { %> 
									<li class="c-sidebar-nav-item" style="background-color: black; user-select: auto;">
										<a class="c-sidebar-nav-link" href="<%=request.getContextPath()%>/admin/newsite.jsp">+ Add Site
										</a>
										</li> 
									<%}%>
						</ul>
        ...
        <script type="text/javascript">
            $('#global_search_text').keyup(function(event) {
                if(event.which === 13){
                    $('body').append('<form method="post" id="frm_global_search" action="<%=request.getContextPath()%>/admin/search.jsp" ><input type="hidden" name="global_search" value="'+$('#global_search_text ').val().trim()+'"/><input type="hidden" name="status" value="All"/></form>');
			...
			callback: function (result) {
				...
				if (result) {
					...
					.always(function (jqXHR) {
						...
						if(jqXHR.status == 401)
						{
							window.location.href = "<%=request.getContextPath()%>/admin/gestion.jsp?_tm=<%=System.currentTimeMillis()%>";
						}
					});
				}
			}
	...
	function addToShortcut(pageName){
		$.ajax({
			url: '<%=request.getContextPath()%>/admin/catalogs/foldersAjax.jsp', type: 'POST', dataType: 'json',
			data: {
