...
<%!
	...
	String getStatus(com.etn.beans.Contexte Etn, String id)
	{
		...
		Set rs = Etn.execute("select * from "+proddb+".deliveryfees where id = " + escape.cote(id));
		if(rs.rs.Rows == 0)
		{
			return status;
		}
		status = "PUBLISHED";
		rs = Etn.execute("select case when p1.version = p2.version then '1' else '0' end from deliveryfees p1, "+proddb+".deliveryfees p2 where p1.id = p2.id and p1.id = " + escape.cote(id));
		rs.next();
...
<%
	...
	{
		Set rsp = Etn.execute("select * from "+com.etn.beans.app.GlobalParm.getParm("PROD_DB")+".deliveryfees WHERE id = "+ escape.cote(delete_id));
		if(rsp.rs.Rows > 0)
		{
			errmsg = "Delivery Fee is already in production. Cannot proceed with delete. You must delete from production first";
		}
		else
		{					
			Set rs_name = Etn.execute("select name from deliveryfees where id = " + escape.cote(delete_id));
			rs_name.next();

			Etn.executeCmd("UPDATE deliveryfees_tbl SET is_deleted='1',updated_on=NOW()"+
				",updated_by="+escape.cote(logedInUserId)+" WHERE id = "+escape.cote(delete_id));

			ActivityLog.addLog(Etn,request,parseNull(session.getAttribute("LOGIN")),delete_id,"DELETED","Delivery Fee",parseNull(rs_name.value(0)),selectedsiteid);
		}
	}

	String q = "select s.*, lg.name as last_updated_by from deliveryfees s left outer join login lg on lg.pid = s.updated_by WHERE s.site_id = "+escape.cote(selectedsiteid);
	if(edit_id.length()>0){
...
breadcrumbs.add(new String[]{"Delivery Fees", ""});
%>
...
<div class="c-wrapper c-fixed-components">
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
			...
			<div class="btn-toolbar mb-2 mb-md-0">
				...
				<div class="btn-group mr-2" role="group" aria-label="...">
					...
					<button type="button" class="btn btn-secondary ml-2 d-flex justify-content-center align-items-center" onclick="addToShortcut('Delivery Fees');" title="Add to shortcuts">
	...
	<form name='frm' id='frm' method='post' action='deliveryfees.jsp' >
			<input type='hidden' value='0' name='delete_id' />
		<% if(errmsg.length() > 0) { %>
		<div class="alert alert-danger" role="alert"><%=errmsg%></div>
		<% } %>
			<div id="save" style="display: none;" class="col-md-12 alert alert-success" role="alert">Delivery Fee is saved.</div>
                        <div id="edit" style="display: none;" class="col-md-12 alert alert-success" role="alert">Modifications have been saved.</div>
			<table id="deliveryfeesTable" class="table table-hover table-vam" style="width:100%;">
				<thead class="thead-dark">
					<th scope="col"><input type='checkbox' id='sltall' value='1' /></th>
					<th scope="col">Order</th>
					<th scope="col">Delivery Fee name</th>
				...
				<tbody class='sortable'>
					...
					while(rs.next()) {
						...
						if(rspw.next())
						{
							...
							nextpublish += "<br><a style='color:black' href='"+request.getContextPath()+"/admin/cancelaction.jsp?type=deliveryfee&id="+rs.value("id")+"&goto=catalogs/commercialoffers/delivery/deliveryfees.jsp'>Cancel "+__ty+"</a>";
					...
					%>
						<tr class="table-<%=rowColor%>" style="cursor: move;">
							...
							%>
								<button type="button" class="btn btn-danger btn-sm" onclick="onbtnclickpublish('<%=rs.value("id")%>', 'deliveryfees', 'delete')" id='unpublishtoprodbtn' data-toggle="tooltip" data-placement="top" title="Unpublish"><i data-feather="x"></i></button>
							<%
								}
							%>
	...
	<!-- Modal add -->
	<div class="modal fade" id="add_new" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="Delivery Fee" aria-hidden="true">
	...
	$(document).ready(function() {
		...
		onpublish=function(pType)
		{
			if ($(".slt_option:checked").length > 0)
			{
				...
				onbtnclickpublish(ids, "deliveryfees", pType);
			}
			else
			{
		...
		onDeliveryFeeSave=function()
		{
			...
			var flags = [];
                        ...
                        $('#deliveryfeesTable tbody tr').each(function(i, tr){
		...
		Sortable.create(document.querySelector('#deliveryfeesTable tbody.sortable'), {
			animation: 150,
			scroll: true,
		...
		$('#deliveryfeesTable tbody tr input.order_seq')
			.keyup(onKeyOrderSeq)
			.blur(setRowOrder);
				...
				.done(function(json) {
					...
					json.forEach(function(tag) {
						...
						suggestion.addEventListener('mousedown', function(e) {
							...
							document.getElementById("applied_to_key").value=docEle.id;
