...
<%!
	...
	String getStatus(com.etn.beans.Contexte Etn, String id)
	{
		...
		Set rs = Etn.execute("select * from "+proddb+".quantitylimits where id = " + escape.cote(id));
		if(rs.rs.Rows == 0)
		{
			return status;
		}
		status = "PUBLISHED";
		rs = Etn.execute("select case when p1.version = p2.version then '1' else '0' end from quantitylimits p1, "+proddb+".quantitylimits p2 where p1.id = p2.id and p1.id = " + escape.cote(id));
		rs.next();
...
<%
	...
	{
		Set rsp = Etn.execute("select * from "+com.etn.beans.app.GlobalParm.getParm("PROD_DB")+".quantitylimits WHERE id = "+ escape.cote(delete_id));
		if(rsp.rs.Rows > 0)
		{
			errmsg = "Quantity limit is already in production. Cannot proceed with delete. You must delete from production first";
		}
		else
		{					
			Set rs_name = Etn.execute("select name from quantitylimits where id = " + escape.cote(delete_id));
			rs_name.next();

			
			Etn.executeCmd("UPDATE quantitylimits_tbl SET is_deleted='1',updated_on=now()"+
				",updated_by="+escape.cote(logedInUserId)+" WHERE id = "+escape.cote(delete_id));

			ActivityLog.addLog(Etn,request,parseNull(session.getAttribute("LOGIN")),delete_id,"DELETED","Quantity Limit",parseNull(rs_name.value(0)),selectedsiteid);
		}
	}

	String q = "select s.*, lg.name as last_updated_by from quantitylimits s left outer join login lg on lg.pid = s.updated_by WHERE s.site_id = "+escape.cote(selectedsiteid);
    if(edit_id.length()>0){
...
<div class="c-wrapper c-fixed-components">
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
			<div>
				<h1 class="h2">Quantity Limits</h1>
				<p class="lead"></p>
			</div>
			...
			<div class="btn-toolbar mb-2 mb-md-0">
				...
				<button type="button" class="btn btn-secondary ml-2 d-flex justify-content-center align-items-center" onclick="addToShortcut('Quantity Limits');" title="Add to shortcuts">
	...
	<form name='frm' id='frm' method='post' action='quantitylimits.jsp' >
			<input type='hidden' value='0' name='delete_id' />

		<% if(errmsg.length() > 0) { %>
		<div class="alert alert-danger" role="alert"><%=errmsg%></div>
		<% } %>
			<div id="save" style="display: none;" class="col-md-12 alert alert-success" role="alert">Quantity Limit is saved.</div>
                        <div id="edit" style="display: none;" class="col-md-12 alert alert-success" role="alert">Modifications have been saved.</div>
			<table id="mainTable" class="table table-hover table-vam" style="width:100%;">
				<thead class="thead-dark">
					<th scope="col"><input type='checkbox' id='sltall' value='1' /></th>
					<th scope="col">Order</th>
					<th scope="col">Quantity Limit name</th>
				...
				<tbody class='sortable'>
					...
					while(rs.next()) {
						...
						if(rspw.next())
						{
							...
							nextpublish += "<br><a style='color:black' href='"+request.getContextPath()+"/admin/cancelaction.jsp?type=quantitylimit&id="+rs.value("id")+"&goto=catalogs/commercialoffers/quantitylimits/quantitylimits.jsp'>Cancel "+__ty+"</a>";
					...
					%>
						<tr class="table-<%=rowColor%>" style="cursor: move;">
							...
							%>
								<button type="button" class="btn btn-danger btn-sm" onclick="onbtnclickpublish('<%=rs.value("id")%>', 'quantitylimits', 'delete')" id='unpublishtoprodbtn' data-toggle="tooltip" data-placement="top" title="Unpublish"><i data-feather="x"></i></button>
							<%
								}
							%>
	...
	<!-- Modal add -->
	<div class="modal fade" id="add_new" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true">
		<div class="modal-dialog modal-lg modal-dialog-slideout" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Quantity Limit</h5>
...
<script>
	$(document).ready(function() {
		...
		onpublish=function(pType)
		{
			if ($(".slt_option:checked").length > 0)
			{
				...
				onbtnclickpublish(ids, "quantitylimits", pType);
			}
			else
			{
        ...
        function editModal(id){
            $.ajax({
                url : '../ajax/quantitylimits.jsp',
                type: 'POST',
                dataType: 'HTML',
                data: {
                    "action": 'quantitylimits',
                    "id": id
                },
		...
		onQuantityLimitSave=function()
		{
			...
			if($("#fee").val() == "") {
				...
				$("#fee").parent().find(".invalid-feedback").css("display","block");
				flag = true;
			}
			else $("#fee").parent().find(".invalid-feedback").css("display","none");
			...
			var flags = [];
                        ...
                        $('#quantitylimitsTable tbody tr').each(function(i, tr){
		...
		setRowOrder = function(){
			...
			$('#mainTable tbody tr').each(function(i,tr){
		...
		Sortable.create(document.querySelector('#mainTable tbody.sortable'), {
			animation: 150,
			scroll: true,
		...
		$('#mainTable tbody.sortable').sortable({
  				axis : "y",
  				scroll : true,
  				scrollSensitivity : 100,
  				scrollSpeed : 30,
		...
		$('#mainTable tbody tr input.order_seq')
			.keyup(onKeyOrderSeq)
			.blur(setRowOrder);
    ...
    function addProduct(){
    	...
    	if($("#applied_to_type_sl").val()=="all" || prodVal.length > 0){

	    	$("#selected_prod_hvng").append('<div style="margin-left: 20px; margin-top: 10px;"><button class="btn btn-pill btn-block btn-secondary" type="button"><strong onclick="removeAppliedTo(this)" style="color:#f16e00; cursor: pointer;">X</strong> ' + ($("#applied_to_type_sl").val()=="all"?"All":prodVal) + '</button> <input type="hidden" name="applied_to_value" value="' + prodKey + '" /> <input type="hidden" name="applied_to_type" value="' + $("#applied_to_type_sl").val() + '" /> </div>');
