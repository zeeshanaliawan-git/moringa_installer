...
%>
<%@ page import="com.etn.sql.escape, com.etn.lang.ResultSet.Set, com.etn.asimina.util.ActivityLog, com.etn.asimina.util.BlockedUserConfig, org.json.JSONArray,org.json.JSONObject, com.etn.asimina.util.UIHelper"%>
...
<%
    ...
    if(!ipOrUsername.equals("")){
        ...
        ActivityLog.addLog(Etn, request, parseNull(session.getAttribute("LOGIN")), Etn.getId()+"","Un-Block "+selectCol,"Blocked User",parseNull(session.getAttribute("LOGIN")), selectedsiteid);
    }
    //get unblock user/ip
    Set rs =  Etn.execute("SELECT *  FROM "+tableName+"  WHERE attempt >= "+escape.cote(nAttempts)+" AND adddate(tm, interval "+escape.cote(String.valueOf(UIHelper.convertTimeUnitToMinutes(parseInt(blockTime), blockTimeUnit)))+" minute) > now()");
