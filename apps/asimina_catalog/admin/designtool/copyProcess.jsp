...
<%
	...
	String copyAs =  parseNull(request.getParameter("copyAs"));
	String message = "";
	...
	if(!isError)
	{
		...
		qry += " select "+escape.cote(copyAs)+", phase, type, text, stepNb, actors, activities, priority, execute, visc, isManual, displayName, rulesVisibleTo, oprType, reverse, topLeftX, topLeftY ";
		qry += " from phases where process = "+escape.cote(process)+"";
		Etn.executeCmd(qry);

		qry = " insert into rules (start_proc, start_phase, errCode, next_proc, next_phase, nstate, action, priorite, rdv, type) ";
		qry += " select "+escape.cote(copyAs)+", start_phase, errCode, "+escape.cote(copyAs)+", next_phase, nstate, action, priorite, rdv, type ";
		qry += " from rules where start_proc = next_proc and start_proc = "+escape.cote(process)+" ";
		Etn.executeCmd(qry);

		qry = " insert into rules (start_proc, start_phase, errCode, next_proc, next_phase, nstate, action, priorite, rdv, type) ";
		qry += " select "+escape.cote(copyAs)+", start_phase, errCode, next_proc, next_phase, nstate, action, priorite, rdv, type ";
		qry += " from rules where start_proc != next_proc and start_proc = "+escape.cote(process)+" ";
		Etn.executeCmd(qry);

		qry = " insert into external_phases (start_proc, next_proc, next_phase, width, height, topLeftX, topLeftY) ";
		qry += " select "+escape.cote(copyAs)+", next_proc, next_phase, width, height, topLeftX, topLeftY ";
		qry += " from external_phases where start_proc = "+escape.cote(process)+"";
		Etn.executeCmd(qry);

		qry = " insert into coordinates (process, profile, phase, topLeftX, topLeftY, width, height) ";
		qry += " select "+escape.cote(copyAs)+", profile, phase, topLeftX, topLeftY, width, height ";
		qry += " from coordinates where process = "+escape.cote(process)+"" ;
		Etn.executeCmd(qry);

		Set rsNew = Etn.execute("select * from rules where start_proc = "+escape.cote(copyAs)+" and priorite = 0");
		while(rsNew.next())
		{
