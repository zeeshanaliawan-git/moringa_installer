...
<%@ page import="java.util.LinkedHashMap, java.io.*, com.etn.beans.app.GlobalParm, com.etn.sql.escape"%>
...
<%
	...
	String[] jsonUuids = request.getParameterValues("jsonUuid");
    
	
	String allJsonIds= "";
	for(String jsonUuid : jsonUuids)
	{
		Set rs = Etn.execute("SELECT * FROM expert_system_json WHERE json_uuid = "+escape.cote(jsonUuid));
		if(rs.next())
		{
			String jsonId = rs.value("id");
			if((new File(GlobalParm.getParm("EXPERT_SYSTEM_GENERATE_SCRIPT_FOLDER") + "/expsys_ui_"+(jsonId.replaceAll("/", "").replaceAll("\\\\", ""))+".js")).length() > 0) 
				out.write("<script src=\""+request.getContextPath()+"/"+GlobalParm.getParm("EXPERT_SYSTEM_GENERATED_JS_URL")+"/expsys_ui_"+jsonId+".js\"></script>\n");		
			if((new File(GlobalParm.getParm("EXPERT_SYSTEM_GENERATE_SCRIPT_FOLDER") + "/expsys_rules_"+(jsonId.replaceAll("/", "").replaceAll("\\\\", ""))+".js")).length() > 0) 
				out.write("<script src=\""+request.getContextPath()+"/"+GlobalParm.getParm("EXPERT_SYSTEM_GENERATED_JS_URL")+"/expsys_rules_"+jsonId+".js\"></script>\n");		
...
//		out.write("<script src=\""+request.getContextPath()+"/js/d3.layout.js\"></script>\n");	
	...
	for(String jsonUuid : jsonUuids)
	{
		Set _rs1 = Etn.execute("SELECT * FROM expert_system_json WHERE json_uuid = "+escape.cote(jsonUuid));
		if(_rs1.next())
		{
			String jsonId = _rs1.value("id");
			String o = "\t\tif(jsonid == '"+jsonId+"') {\n";
			o += "\t\t\tif(typeof(_uiDisplay"+jsonId+") == 'function') _uiDisplay"+jsonId+"(_json);\n";
			o += "\t\t\tif(typeof(_applyExpertSystemRules"+jsonId+") == 'function') _applyExpertSystemRules"+jsonId+"(_json);\n";
			o += "\t\t}\n";
			out.write(o);
		}
	}
