		var globalEnable = false;
                ...
                function addToCart(id, cookieName, path, isTarif){
                    ...
                    if(productsArray.some(function(e){ return e.id == id })){
                        jeuneError();
                        return;
                    } else {
                        productsArray.push(product);
                ...
                function updatePriceAjax(path, postData, enable){
                    jQuery.ajax({
			url : path+'cart/jeuneattributeprice.jsp',
			type: 'POST',
			dataType: 'json',
			data: postData,
			success : function(data)
			{
                            ...
                            if(data.installment_price!=""){
                                var jeune_prefix = "";
                                if(data.installment_price>0) jeune_prefix = data.installment_price+"<br>";
                                $("#currency_frequency").html("");
                                $("#currency_frequency_logged").html("");
                                if($("#promo_price_display").length){
                                    $("#promo_price_display").html(jeune_prefix+data.recurring_price);
                                    $("#price_display").hide();
                                } 
                                else{
                                    $("#price_display").html(jeune_prefix+data.recurring_price);
                                    $("#price_display").show();
                                } 
                                
                                if($("#promo_price_display_logged").length){
                                    $("#promo_price_display_logged").html(jeune_prefix+data.recurring_price);
                                    $("#price_display_logged").hide();
                                } 
                                else{
                                    $("#price_display_logged").html(jeune_prefix+data.recurring_price);
                            ...
                            if(enable && $("#quantity").length&&parseInt($("#quantity").val())>parseInt($("#stock").html())) { // so that it only checks when other checks passed
