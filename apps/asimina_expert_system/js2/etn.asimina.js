...
(function (util){
    ...
    util.getCollapse = function(collapseId,label){
        var button = util.createElement("button",
                                {"type":"button","class":"btn btn-primary btn-lg btn-block text-left mb-2","data-toggle":"collapse","href":"#" + collapseId,"role":"button","aria-expanded":"false","aria-controls":collapseId},
                                null,label);

        var div = util.createElement("div",{"class":"collapse p-3 mb-2 show","id":collapseId});
    ...
    util.getFormRow = function(label,fields){
        ...
        $(row).append('<div class="col-sm-9"/>');
        if(Array.isArray(fields)){
            $(row).find('div.col-sm-9').append('<div class="input-group"/>');
            fields.forEach(function(f){
                $(row).find('div.col-sm-9').find("div.input-group").append(f);
                $(row).find('div.col-sm-9').append("<div class='invalid-feedback'/>");
            });
        }else{
            $(row).find('div.col-sm-9').append(fields);
            $(row).find('div.col-sm-9').append("<div class='invalid-feedback'/>");
        }
        return row;
    };
...
(function( tabs , util) {
    ...
    var tabView = function(tabId,divParent,languageId,tabData){
        var collapse = util.getCollapse("productTabs" + tabId,"Tab " + (_count + 1));


        var btnGroup = util.createElement("div",{"class":"btn-group col-12 px-0"});
...
(function( blocks , util) {
    ...
    var blockView = function(blockId,divParent,languageId,blockData){
        ...
        if(blockData){
            ...
            etn.asimina.images.add("description_essential_block_image_lang_" + languageId,collapse.div,blockData,"Image");
        }else{
            etn.asimina.images.add("description_essential_block_image_lang_" + languageId,collapse.div,null,"Image");
        }

        var ckeditorInstance = CKEDITOR.replace(text);
...
(function( langtabs ) {
    ...
    function setup(){
        ...
        var divContent = createElement("div",{"class":"tab-content"});
...
(function( variants ) {
    ...
    var variantView = function(id,selector,variantData,index,isPriceTaxIncluded,taxPercentage,constants){
        ...
        var divHeading = createElement("div",{"class":"btn-group col-12 variantHeading","role":"group","aria-label":"Basic example"},{"padding-left":"0px","padding-right":"0px"});

        var buttonHeading = createElement("button",
                                            {"class":"btn btn-success btn-lg btn-block text-left mb-2","type":"button","data-toggle":"collapse","href":"#collapse" + id,"role":"button","aria-expanded":"false","aria-controls":"collapse" + id},
                                            null,getHeadingText());
        ...
        $(divHeading).append(buttonHeading);
        $(divHeading).append(deleteVariantBtn);
        ...
        $(divHeading).append('<div class="input-group-text mb-2"/>');
        $(divHeading).find("div.input-group-text").append(fieldDefault);
        $(divHeading).find("div.input-group-text").append(fieldDefaultHidden);

        var divBody = createElement("div",{"class":"collapse p-3 mb-2","id":"collapse" + id});

        var fieldStock = createElement("input",{"type":"hidden","id":"variantStock" + id,"name":"variantStock"});
        $(divBody).append(fieldStock);

        var fieldId = createElement("input",{"type":"hidden","id":"variantId" + id,"name":"variantId","value":id});
        $(divBody).append(fieldId);
        ...
        _options.catalogAttributes.forEach(function(a){
            ...
            $(divBody).append(getFormRow(a.name,field));
        });

        var fieldSKU = getRequiredTextField("variantSKU");
        $(fieldSKU).val(getValue("sku"));
        $(divBody).append(getFormRow("SKU",fieldSKU));
        ...
        $(divBody).append(getFormRow("Activated",fieldActive));
        ...
        if(constants) {
            ...
            $(divBody).append(getFormRow("Sticker",fieldSticker));
        ...
        $(divBody).append(getFormRow("Variant name",fieldNames));
        ...
        if(_options.catalogType === "offer"){
            ...
            $(divBody).append(getFormRow("Variant main features",fieldMainFeatures));
            $.each(fieldMainFeatures, function(index, field) {
                var ckeditorInstance = CKEDITOR.replace($(field).find("textarea").get(0));
                ckeditorInstance.on("instanceReady",function(evt) {
                    //console.log("ckeditor ready "+ evt.editor.name);
                });
                ckeditorInstance.on("blur",function(evt){
            ...
            langs.forEach(function(lang){
                $(divBody).append(createElement("input",{"id": "variantMainFeatures" + id,"name":"variantMainFeatures","type":"hidden","class":""},null,null,lang));
            });
        }
        ...
        $(priceCollapse.div).append(getFormRow("Display / Price / price after tax<span class='offer_postpaid_show' > / frequency</span>",[showPriceDiv,fieldPrice,fieldPriceAfterTax,fieldFrequency]));
        ...
        $(priceCollapse.div).append(commitmentRow);
...
//        });
        ...
        $(divBody).append(priceCollapse.button);
        $(divBody).append(priceCollapse.div);
        ...
        for(var i=0; i < langs.length; i++){
            ...
            var variantButtonDesktopLabel = createElement("input",{"id": "variantButtonDesktopLabel" + id , "name":"variantButtonDesktopLabel", "placeholder": "Label", "class":"form-control "},null,null,langs[i]);
            var variantButtonDesktopUrl = createElement("input",{"id": "variantButtonDesktopUrl" + id, "name":"variantButtonDesktopUrl", "placeholder": "URL", "class":"form-control "},null,null,langs[i]);
            var variantButtonMobileLabel = createElement("input",{"id": "variantButtonMobileLabel" + id, "name":"variantButtonMobileLabel", "placeholder": "Label", "class":"form-control "},null,null,langs[i]);
            var variantButtonMobileUrl = createElement("input",{"id": "variantButtonMobileUrl" + id, "name":"variantButtonMobileUrl", "placeholder": "URL" ,"class":"form-control "},null,null,langs[i]);
            ...
            $(variantButtonDesktopUrlGenerator).text("Generator");
            $(variantButtonMobileUrlGenerator).text("Generator");

            $(divActionButton).append(getFormRow("Button desktop / URL",[variantButtonDesktopLabel, variantButtonDesktopUrl, variantButtonDesktopUrlGenerator]));
            $(divActionButton).append(getFormRow("Button mobile / URL",[variantButtonMobileLabel, variantButtonMobileUrl, variantButtonMobileUrlGenerator]));

            $(actionButtonCollapse.div).append(divActionButton);
        }

        $(divBody).append(actionButtonCollapse.button);
        $(divBody).append(actionButtonCollapse.div);

        if(variants.getProductType() === "product"){
            ...
            for(var i=0; i < langs.length; i++){
                ...
                $(imagesCollapse.div).append(divImage);
                ...
                variantData.resources.forEach(function(resource){
                    if(resource.type === "image"){
                        ...
                        if(languageId && divImages[languageId]){
                            etn.asimina.images.add(getImageName(languageId),divImages[languageId],resource);
                        }
                    }
                });
            }
            ...
            Object.keys(divImages).forEach(function(languageId){
                etn.asimina.images.addEmpty(getImageName(languageId),divImages[languageId],3);
            }); //adds the remaining image blocks upto 3


            $(divBody).append(imagesCollapse.button);
            $(divBody).append(imagesCollapse.div);

        }

        $(selector).append(divHeading);
        $(selector).append(divBody);
