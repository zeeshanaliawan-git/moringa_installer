...
import com.etn.asimina.util.CommonHelper;
...
public class Authenticate implements Filter
{
	...
	public void doFilter(ServletRequest _request, ServletResponse _response, FilterChain chain) throws IOException, ServletException
	{
		...
		if(Etn.getId() == 0)
		{
			...
			Logger.info("ExpSys::Authenticate","Webapp token : "+webappToken);				
			if(webappToken.length() > 0)
			{
				...
				if(rsW.next())
				{
					...
					Logger.info("ExpSys::Authenticate","accessid : "+accessid);
					String catalogsessionid = rsW.value("catalog_session_id");
					Logger.info("ExpSys::Authenticate","catalogsessionid : "+catalogsessionid);
					...
					if(rs.next())
					{
						...
						if(Integer.parseInt(timediff) > (Integer.parseInt(timeout) * 60))
						{
							...
							Logger.info("ExpSys::Authenticate", "Session timeout");
						}
						else
						{
							Etn.setContexte(rs.value("name"),rs.value("pass"));
							Etn.executeCmd("insert into "+GlobalParm.getParm("COMMONS_DB")+".user_sessions (catalog_session_id, expsys_session_id) values ("+escape.cote(catalogsessionid)+","+escape.cote(""+request.getSession().getId())+") on duplicate key update last_updated_on = now(), expsys_session_id = "+escape.cote(""+request.getSession().getId()));
							gotogestion = false;
						}
					}
				...
				{
					Logger.info("ExpSys::Authenticate", "Webapp token is expired");
				}
			}

			Logger.info("ExpSys::Authenticate","gotogestion : "+gotogestion);			
			...
			if(gotogestion)
			{
				...
				String redirectUrl = GlobalParm.getParm("GOTO_EXPSYS_APP_URL");
		...
		if(Etn.getId() > 0)
		{
			...
			if(rs.next() && CommonHelper.parseNull(rs.value("allowed_ips")).length() > 0)
			{
				canContinue = false;
				List<String> allowedIps = Arrays.asList(CommonHelper.parseNull(rs.value("allowed_ips")).split(","));
				if(allowedIps.contains(ip)) canContinue = true;				
			}
			
			Logger.info("ExpSys::Authenticate","Etn.getId:"+Etn.getId()+" canContinue:"+canContinue+" IP:"+ip);
		...
		if("TEST_SITE_ACCESS".equalsIgnoreCase(CommonHelper.parseNull(request.getSession().getAttribute("PROFIL"))) || "PROD_SITE_ACCESS".equalsIgnoreCase(CommonHelper.parseNull(request.getSession().getAttribute("PROFIL"))))
		{
			response.sendRedirect(request.getContextPath() + "/unauthorized.jsp");
			return;
		}
