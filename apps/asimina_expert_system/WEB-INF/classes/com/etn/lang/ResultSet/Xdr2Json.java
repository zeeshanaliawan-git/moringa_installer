...
throws IOException
{
	return  getJson( rs , clef, fmt, ignoreColumnNames, false);
}
public int getJson( Set rs , String clef, int fmt, boolean ignoreColumnNames, boolean reactCall)
throws IOException
{ 
   if( fmt == 0 && rs != null ) 
     fmt=( rs.rs.Rows > 1 ? 2 : 1 );

   if(fmt == 2 && reactCall) fmt = 20;
   ...
   case 7 : if(d3MapJsonChl(rs) != 0)
                wr("null");
            break;
   case 20 : wReactJsonArray(rs); break;
