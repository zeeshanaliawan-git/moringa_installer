        ...
        try{
            ...
            if(postId.length()>0){
                try{
					...
					Set rs2 = Etn.execute("select * from client_review_reactions where post_id="+escape.cote(postId)+" and client_id="+escape.cote(clientId)+" and is_like= 0");
					
					if(rs2.rs.Rows>0){
						Etn.executeCmd("delete from client_review_reactions where post_id = "+escape.cote(postId)+" and client_id="+com.etn.sql.escape.cote(clientId)+" and is_like=0 ");
					}else{
						Etn.executeCmd("INSERT INTO client_review_reactions (post_id,client_id,is_like) VALUES ("+escape.cote(postId)+","+escape.cote(clientId)+",0) ON DUPLICATE KEY UPDATE is_like=0");
					}
